...
void Emitter::add_lsl(arm64::Reg rd, arm64::Reg rn, arm64::Reg rm, unsigned shift) {
...
void Emitter::msub(arm64::Reg rd, arm64::Reg rn, arm64::Reg rm, arm64::Reg ra) {
  impl_->masm.Msub(to_vixl_x(rd), to_vixl_x(rn), to_vixl_x(rm), to_vixl_x(ra));
}
...
void Emitter::load(arm64::Reg rd, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Ldrb(to_vixl_x(rd), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Ldrh(to_vixl_x(rd), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Ldr(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Ldr(to_vixl_x(rd), mo);
      return;
  }
}
...
void Emitter::store(arm64::Reg rv, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Strb(to_vixl_x(rv), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Strh(to_vixl_x(rv), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Str(to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Str(to_vixl_x(rv), mo);
      return;
  }
}
...
void Emitter::load_acquire(arm64::Reg rd, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Ldarb(to_vixl_x(rd), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Ldarh(to_vixl_x(rd), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Ldar(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Ldar(to_vixl_x(rd), mo);
      return;
  }
}
...
void Emitter::store_release(arm64::Reg rv, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Stlrb(to_vixl_x(rv), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Stlrh(to_vixl_x(rv), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Stlr(to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Stlr(to_vixl_x(rv), mo);
      return;
  }
}
...
void Emitter::ldxr(arm64::Reg rd, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Ldxrb(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Ldxrh(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Ldxr(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Ldxr(to_vixl_x(rd), mo);
      return;
  }
}

void Emitter::stxr(arm64::Reg rs, arm64::Reg rv, arm64::Reg raddr, ir::OpSize size) {
  const vixl_aa::MemOperand mo(to_vixl_x(raddr));
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Stxrb(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Stxrh(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Stxr(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Stxr(to_vixl_w(rs), to_vixl_x(rv), mo);
      return;
  }
}
...
void Emitter::ldaxr(arm64::Reg rd, arm64::Reg raddr, ir::OpSize size) {
  ...
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Ldaxrb(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Ldaxrh(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Ldaxr(to_vixl_w(rd), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Ldaxr(to_vixl_x(rd), mo);
      return;
  }
}

void Emitter::stlxr(arm64::Reg rs, arm64::Reg rv, arm64::Reg raddr, ir::OpSize size) {
  const vixl_aa::MemOperand mo(to_vixl_x(raddr));
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Stlxrb(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Stlxrh(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Stlxr(to_vixl_w(rs), to_vixl_w(rv), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Stlxr(to_vixl_w(rs), to_vixl_x(rv), mo);
      return;
  }
}
...
void Emitter::stlxp(arm64::Reg rs, arm64::Reg rv_low, arm64::Reg rv_high, arm64::Reg raddr) {
  const vixl_aa::MemOperand mo(to_vixl_x(raddr));
...
void Emitter::casal(arm64::Reg rs, arm64::Reg rt, arm64::Reg raddr, ir::OpSize size) {
  const vixl_aa::MemOperand mo(to_vixl_x(raddr));
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Casalb(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Casalh(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Casal(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Casal(to_vixl_x(rs), to_vixl_x(rt), mo);
      return;
  }
}

void Emitter::ldaddal(arm64::Reg rs, arm64::Reg rt, arm64::Reg raddr, ir::OpSize size) {
  const vixl_aa::MemOperand mo(to_vixl_x(raddr));
  switch (size) {
    case ir::OpSize::I8:
      impl_->masm.Ldaddalb(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I16:
      impl_->masm.Ldaddalh(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I32:
      impl_->masm.Ldaddal(to_vixl_w(rs), to_vixl_w(rt), mo);
      return;
    case ir::OpSize::I64:
      impl_->masm.Ldaddal(to_vixl_x(rs), to_vixl_x(rt), mo);
      return;
  }
}

void Emitter::load_offset(arm64::Reg rd, arm64::Reg rbase, std::int32_t imm) {
  impl_->masm.Ldr(to_vixl_x(rd), vixl_aa::MemOperand(to_vixl_x(rbase), imm));
}

void Emitter::store_offset(arm64::Reg rv, arm64::Reg rbase, std::int32_t imm) {
  impl_->masm.Str(to_vixl_x(rv), vixl_aa::MemOperand(to_vixl_x(rbase), imm));
}

void Emitter::sp_load(arm64::Reg rd, std::int32_t imm) {
  impl_->masm.Ldr(to_vixl_x(rd), vixl_aa::MemOperand(vixl_aa::sp, imm));
}

void Emitter::sp_store(arm64::Reg rv, std::int32_t imm) {
  impl_->masm.Str(to_vixl_x(rv), vixl_aa::MemOperand(vixl_aa::sp, imm));
}
...
vixl_aa::Condition to_vixl_cond(ir::CondCode cc) noexcept {
  switch (cc) {
    ...
    case ir::CondCode::Ult:
      return vixl_aa::lo;  // unsigned <
    case ir::CondCode::Ule:
      return vixl_aa::ls;  // unsigned <=
    case ir::CondCode::Ugt:
      return vixl_aa::hi;  // unsigned >
    case ir::CondCode::Uge:
      return vixl_aa::hs;  // unsigned >=
    case ir::CondCode::Slt:
      return vixl_aa::lt;
    case ir::CondCode::Sle:
      return vixl_aa::le;
    case ir::CondCode::Sgt:
      return vixl_aa::gt;
    case ir::CondCode::Sge:
      return vixl_aa::ge;
    case ir::CondCode::Cc:
      return vixl_aa::cc;  // carry clear (unsigned <)
    case ir::CondCode::Nc:
      return vixl_aa::cs;  // carry set   (unsigned >=)
    case ir::CondCode::Ov:
      return vixl_aa::vs;  // overflow set
    case ir::CondCode::NoOv:
      return vixl_aa::vc;  // overflow clear
    case ir::CondCode::Mi:
      return vixl_aa::mi;  // sign set (negative)
    case ir::CondCode::Pl:
      return vixl_aa::pl;  // sign clear (non-negative)
  }
...
void Emitter::count_zero_flags(arm64::Reg result, arm64::Reg src, arm64::Reg w_tmp) {
...
void Emitter::csel(arm64::Reg rd, arm64::Reg rn_true, arm64::Reg rn_false, ir::CondCode cc) {
  impl_->masm.Csel(to_vixl_x(rd), to_vixl_x(rn_true), to_vixl_x(rn_false), to_vixl_cond(cc));
}
...
void Emitter::fmov_v_from_x(FpReg rd, arm64::Reg rn, ir::FpSize sz) {
  const vixl_aa::Register r =
      sz == ir::FpSize::F32 ? vixl_aa::Register(to_vixl_w(rn)) : vixl_aa::Register(to_vixl_x(rn));
  if (sz == ir::FpSize::F32) {
...
void Emitter::fmov_x_from_v(arm64::Reg rd, FpReg rn, ir::FpSize sz) {
  const vixl_aa::Register r =
      sz == ir::FpSize::F32 ? vixl_aa::Register(to_vixl_w(rd)) : vixl_aa::Register(to_vixl_x(rd));
  if (sz == ir::FpSize::F32) {
...
void Emitter::scvtf(FpReg rd, arm64::Reg rn, ir::OpSize int_sz, ir::FpSize fp_sz) {
  const vixl_aa::Register r = int_sz == ir::OpSize::I32 ? vixl_aa::Register(to_vixl_w(rn))
                                                        : vixl_aa::Register(to_vixl_x(rn));
  const vixl_aa::VRegister v = fp_sz == ir::FpSize::F32 ? vixl_aa::VRegister(to_vixl_s(rd))
                                                        : vixl_aa::VRegister(to_vixl_d(rd));
  impl_->masm.Scvtf(v, r);
}
void Emitter::fcvt_scalar_with_upper(FpReg rd, FpReg lhs, FpReg src, ir::FpSize src_sz,
                                     ir::FpSize dst_sz) {
...
void Emitter::fcvtzs(arm64::Reg rd, FpReg rn, ir::FpSize fp_sz, ir::OpSize int_sz) {
  const vixl_aa::Register r = int_sz == ir::OpSize::I32 ? vixl_aa::Register(to_vixl_w(rd))
                                                        : vixl_aa::Register(to_vixl_x(rd));
  const vixl_aa::VRegister v = fp_sz == ir::FpSize::F32 ? vixl_aa::VRegister(to_vixl_s(rn))
...
vixl_aa::VRegister to_vixl_q(Emitter::FpReg r, Emitter::VecLane lane) noexcept {
  ...
  switch (lane) {
    ...
    case L::H8:
      return vixl_aa::VRegister(code, vixl_aa::kFormat8H);
    case L::S4:
      return vixl_aa::VRegister(code, vixl_aa::kFormat4S);
    case L::D2:
      return vixl_aa::VRegister(code, vixl_aa::kFormat2D);
  }
  return vixl_aa::VRegister(code, vixl_aa::kFormat16B);
}
...
void Emitter::vadd_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Add(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vsub_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Sub(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vand_q(FpReg rd, FpReg rn, FpReg rm) {
  impl_->masm.And(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(rn), to_vixl_q_bitwise(rm));
}
void Emitter::vorr_q(FpReg rd, FpReg rn, FpReg rm) {
  impl_->masm.Orr(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(rn), to_vixl_q_bitwise(rm));
}
void Emitter::veor_q(FpReg rd, FpReg rn, FpReg rm) {
  impl_->masm.Eor(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(rn), to_vixl_q_bitwise(rm));
}
void Emitter::vmul_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Mul(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vsqadd_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Sqadd(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vuqadd_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Uqadd(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vsqsub_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Sqsub(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vuqsub_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Uqsub(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
...
void Emitter::vsubp_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  ...
  //   neg v_t, v_n              ; v_t = -v_n
  //   trn1 v_t,  v_n, v_t        ; alternates v_n.even, v_t.even = even lanes of {a0, -a1, a2,
  //   -a3,...}
...
void Emitter::vfadd_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fadd(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfsub_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fsub(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfmul_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fmul(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfdiv_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fdiv(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfmin_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fmin(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfmax_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fmax(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
...
void Emitter::vfaddp_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Faddp(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
...
void Emitter::vfmla_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fmla(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
void Emitter::vfmls_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Fmls(to_vixl_q_fp(rd, lane), to_vixl_q_fp(rn, lane), to_vixl_q_fp(rm, lane));
}
...
void Emitter::vmov_q(FpReg rd, FpReg rn) {
  if (static_cast<int>(rd) == static_cast<int>(rn))
    return;
...
void emit_scalar_sse_op(vixl_aa::MacroAssembler& masm, int rd_c, int rn_c, int rm_c, ir::FpSize sz,
                        char which) {
  ...
  if (sz == ir::FpSize::F32) {
    ...
    switch (which) {
      case '+':
        masm.Fadd(s_t, s_n, s_m);
        break;
      case '-':
        masm.Fsub(s_t, s_n, s_m);
        break;
      case '*':
        masm.Fmul(s_t, s_n, s_m);
        break;
      case '/':
        masm.Fdiv(s_t, s_n, s_m);
        break;
      case 'm':
        masm.Fmin(s_t, s_n, s_m);
        break;
      case 'M':
        masm.Fmax(s_t, s_n, s_m);
        break;
      case 's':
        masm.Fsqrt(s_t, s_m);
        break;
    ...
    switch (which) {
      case '+':
        masm.Fadd(d_t, d_n, d_m);
        break;
      case '-':
        masm.Fsub(d_t, d_n, d_m);
        break;
      case '*':
        masm.Fmul(d_t, d_n, d_m);
        break;
      case '/':
        masm.Fdiv(d_t, d_n, d_m);
        break;
      case 'm':
        masm.Fmin(d_t, d_n, d_m);
        break;
      case 'M':
        masm.Fmax(d_t, d_n, d_m);
        break;
      case 's':
        masm.Fsqrt(d_t, d_m);
        break;
...
namespace {
void emit_scalar_fma_op(vixl_aa::MacroAssembler& masm, int rd_c, int rupper_c, int ra_c, int rb_c,
                        int rc_c, ir::FpSize sz, bool neg_addend, bool neg_mul) {
...
void Emitter::vfma_scalar(FpReg rd, FpReg rupper, FpReg ra, FpReg rb, FpReg rc, ir::FpSize sz,
                          bool neg_addend, bool neg_mul) {
  emit_scalar_fma_op(impl_->masm, static_cast<int>(rd), static_cast<int>(rupper),
                     static_cast<int>(ra), static_cast<int>(rb), static_cast<int>(rc), sz,
                     neg_addend, neg_mul);
}

void Emitter::vfadd_scalar(FpReg rd, FpReg rn, FpReg rm, ir::FpSize sz) {
  emit_scalar_sse_op(impl_->masm, static_cast<int>(rd), static_cast<int>(rn), static_cast<int>(rm),
                     sz, '+');
}
...
void Emitter::vcmeq_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Cmeq(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vcmgt_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Cmgt(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}

void Emitter::vzip1_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Zip1(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}
void Emitter::vzip2_q(FpReg rd, FpReg rn, FpReg rm, VecLane lane) {
  impl_->masm.Zip2(to_vixl_q(rd, lane), to_vixl_q(rn, lane), to_vixl_q(rm, lane));
}

namespace {
unsigned lane_bits(Emitter::VecLane lane) noexcept {
  switch (lane) {
    case Emitter::VecLane::B16:
      return 8;
    case Emitter::VecLane::H8:
      return 16;
    case Emitter::VecLane::S4:
      return 32;
    case Emitter::VecLane::D2:
      return 64;
  }
  return 8;
}
...
void Emitter::vshl_imm_q(FpReg rd, FpReg rn, std::uint8_t count, VecLane lane) {
  ...
  impl_->masm.Shl(to_vixl_q(rd, lane), to_vixl_q(rn, lane), static_cast<int>(count));
}
void Emitter::vushr_imm_q(FpReg rd, FpReg rn, std::uint8_t count, VecLane lane) {
  ...
  impl_->masm.Ushr(to_vixl_q(rd, lane), to_vixl_q(rn, lane), static_cast<int>(count));
}
void Emitter::vshlb_imm_q(FpReg rd, FpReg rn, std::uint8_t count) {
  ...
  impl_->masm.Ext(to_vixl_q_bitwise(rd), v_t, to_vixl_q_bitwise(rn), static_cast<int>(16 - count));
}
void Emitter::vshrb_imm_q(FpReg rd, FpReg rn, std::uint8_t count) {
  ...
  const vixl_aa::VRegister v_t(kInternalFpScratchV, vixl_aa::kFormat16B);
  impl_->masm.Movi(v_t, 0);
  impl_->masm.Ext(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(rn), v_t, static_cast<int>(count));
}
...
void Emitter::vsshr_imm_q(FpReg rd, FpReg rn, std::uint8_t count, VecLane lane) {
  ...
  impl_->masm.Sshr(to_vixl_q(rd, lane), to_vixl_q(rn, lane), static_cast<int>(eff));
}
...
vixl_aa::VectorFormat vec_format_for_lane(Emitter::VecLane lane) noexcept {
  ...
  switch (lane) {
    ...
    case L::H8:
      return vixl_aa::kFormat8H;
    case L::S4:
      return vixl_aa::kFormat4S;
    case L::D2:
      return vixl_aa::kFormat2D;
  }
  return vixl_aa::kFormat16B;
}
}  // namespace

void Emitter::vins_lane_from_w(FpReg rd, FpReg rn, std::uint8_t lane_idx, arm64::Reg w_value,
                               VecLane lane) {
  ...
  if (lane == VecLane::D2) {
    impl_->masm.Mov(v_t_lane, lane_idx, vixl_aa::Register(to_vixl_x(w_value)));
  } else {
    impl_->masm.Mov(v_t_lane, lane_idx, vixl_aa::Register(to_vixl_w(w_value)));
  }
  const vixl_aa::VRegister v_d_q(static_cast<int>(rd), vixl_aa::kFormat16B);
  impl_->masm.Mov(v_d_q, v_t_q);
}
...
void emit_fp_pred_packed(vixl_aa::MacroAssembler& masm, const vixl_aa::VRegister& v_d,
                         const vixl_aa::VRegister& v_n, const vixl_aa::VRegister& v_m,
                         vixl_aa::VectorFormat fmt, std::uint8_t pred) {
...
void emit_vfrint(vixl_aa::MacroAssembler& masm, const vixl_aa::VRegister& vd,
                 const vixl_aa::VRegister& vn, std::uint8_t mode) {
  switch (mode & 0x3u) {
    case 0:
      masm.Frintn(vd, vn);
      break;  // round to nearest (ties to even)
    case 1:
      masm.Frintm(vd, vn);
      break;  // round down (toward -inf)
    case 2:
      masm.Frintp(vd, vn);
      break;  // round up   (toward +inf)
    case 3:
      masm.Frintz(vd, vn);
...
void Emitter::vptest(FpReg lhs, FpReg rhs, arm64::Reg w_tmp) {
  ...
  impl_->masm.Cset(vixl_aa::WRegister(static_cast<int>(x_b.GetCode())),
                   vixl_aa::ne);  // NOT is_zero_b
  // w_tmp = (x_a << 30) | (x_b << 29).
  impl_->masm.Lsl(to_vixl_w(w_tmp), vixl_aa::WRegister(static_cast<int>(x_a.GetCode())), 30);
  impl_->masm.Orr(
      to_vixl_w(w_tmp), to_vixl_w(w_tmp),
      vixl_aa::Operand(vixl_aa::WRegister(static_cast<int>(x_b.GetCode())), vixl_aa::LSL, 29));
  // msr nzcv, x_tmp
  impl_->masm.Msr(vixl_aa::NZCV, vixl_aa::Register(to_vixl_x(w_tmp)));
}

void Emitter::vptest_ymm(FpReg lo_lhs, FpReg lo_rhs, FpReg hi_lhs, FpReg hi_rhs, arm64::Reg w_tmp) {
...
void Emitter::vtbl2_q(FpReg dst, FpReg src_lo, FpReg src_hi, FpReg idx) {
  ...
  static_assert(kInternalFpScratchV == kAuxHi, "vtbl2_q assumes kInternalFpScratchV == V31");
...
void Emitter::vpclmulqdq(FpReg dst, FpReg lhs, FpReg rhs, bool lhs_high, bool rhs_high) {
...
void Emitter::crc32c(arm64::Reg rd, arm64::Reg rcrc, arm64::Reg rdata, ir::OpSize data_size) {
  ...
  switch (data_size) {
    case ir::OpSize::I8:
      impl_->masm.Crc32cb(to_vixl_w(rd), to_vixl_w(rcrc), to_vixl_w(rdata));
      break;
    case ir::OpSize::I16:
      impl_->masm.Crc32ch(to_vixl_w(rd), to_vixl_w(rcrc), to_vixl_w(rdata));
      break;
    case ir::OpSize::I32:
      impl_->masm.Crc32cw(to_vixl_w(rd), to_vixl_w(rcrc), to_vixl_w(rdata));
      break;
    case ir::OpSize::I64:
      impl_->masm.Crc32cx(to_vixl_w(rd), to_vixl_w(rcrc), to_vixl_x(rdata));
      break;
  }
}
...
void Emitter::x87_load(arm64::Reg state_ptr, arm64::Reg dst, arm64::Reg scratch_tos,
                       arm64::Reg scratch_slot, std::int32_t array_offset,
                       std::int32_t tos_byte_offset, std::uint8_t st_index) {
  ...
  impl_->masm.Add(to_vixl_x(scratch_slot), to_vixl_x(state_ptr), to_vixl_x(scratch_slot));
  impl_->masm.Ldr(to_vixl_x(dst), vixl_aa::MemOperand(to_vixl_x(scratch_slot), array_offset));
}

void Emitter::x87_store(arm64::Reg state_ptr, arm64::Reg value, arm64::Reg scratch_tos,
                        arm64::Reg scratch_slot, std::int32_t array_offset,
                        std::int32_t tos_byte_offset, std::uint8_t st_index) {
  ...
  impl_->masm.Lsl(to_vixl_x(scratch_slot), to_vixl_x(scratch_tos), 4);
  impl_->masm.Add(to_vixl_x(scratch_slot), to_vixl_x(state_ptr), to_vixl_x(scratch_slot));
  impl_->masm.Str(to_vixl_x(value), vixl_aa::MemOperand(to_vixl_x(scratch_slot), array_offset));
}

void Emitter::x87_push(arm64::Reg state_ptr, arm64::Reg value, arm64::Reg scratch_tos,
                       arm64::Reg scratch_slot, std::int32_t array_offset,
                       std::int32_t tos_byte_offset) {
...
void Emitter::x87_pop(arm64::Reg state_ptr, arm64::Reg dst, arm64::Reg scratch_tos,
                      arm64::Reg scratch_slot, std::int32_t array_offset,
                      std::int32_t tos_byte_offset) {
...
void Emitter::vaes_keygenassist(FpReg dst, FpReg src, std::uint8_t rcon) {
  ...
  if (rcon != 0u) {
    const std::uint64_t rcon_lane = static_cast<std::uint64_t>(rcon) << 32u;
...
void Emitter::vsha1_rnds4(FpReg dst, FpReg a, FpReg b, std::uint8_t sel) {
  ...
  const vixl_aa::VRegister v_dst16(static_cast<int>(dst), vixl_aa::kFormat16B);
  const vixl_aa::VRegister v_dst4s(static_cast<int>(dst), vixl_aa::kFormat4S);
  ...
  static constexpr std::uint32_t kSha1K[4] = {0x5A827999u, 0x6ED9EBA1u, 0x8F1BBCDCu, 0xCA62C1D6u};
  ...
  switch (sel & 3u) {
    case 0:
      impl_->masm.Sha1c(q_state, s_e, v_msg4s);
      break;
    case 2:
      impl_->masm.Sha1m(q_state, s_e, v_msg4s);
      break;
    default:
      impl_->masm.Sha1p(q_state, s_e, v_msg4s);
      break;
  }
...
void Emitter::vsha256_rnds2(FpReg dst, FpReg a, FpReg b, FpReg wk) {
  ...
  const vixl_aa::VRegister v_wk2d(static_cast<int>(wk), vixl_aa::kFormat2D);
  const vixl_aa::VRegister v_dst16(static_cast<int>(dst), vixl_aa::kFormat16B);
  const vixl_aa::VRegister v_dst2d(static_cast<int>(dst), vixl_aa::kFormat2D);
...
void Emitter::mrs_cntvct(arm64::Reg rd) {
  ...
  vixl::ExactAssemblyScope scope(&impl_->masm, vixl_aa::kInstructionSize);
...
void Emitter::vfrint_q(FpReg rd, FpReg rn, ir::FpSize sz, std::uint8_t mode) {
  const auto fmt = (sz == ir::FpSize::F32) ? vixl_aa::kFormat4S : vixl_aa::kFormat2D;
...
void Emitter::vfrint_scalar_with_upper(FpReg rd, FpReg lhs, FpReg rhs, ir::FpSize sz,
                                       std::uint8_t mode) {
  ...
  if (sz == ir::FpSize::F32) {
    ...
    switch (mode & 0x3u) {
      case 0:
        impl_->masm.Frintn(s_t, s_n);
        break;
      case 1:
        impl_->masm.Frintm(s_t, s_n);
        break;
      case 2:
        impl_->masm.Frintp(s_t, s_n);
        break;
      case 3:
        impl_->masm.Frintz(s_t, s_n);
        break;
    }
  } else {
    const vixl_aa::DRegister d_t(kInternalFpScratchV);
    ...
    switch (mode & 0x3u) {
      case 0:
        impl_->masm.Frintn(d_t, d_n);
        break;
      case 1:
        impl_->masm.Frintm(d_t, d_n);
        break;
      case 2:
        impl_->masm.Frintp(d_t, d_n);
        break;
      case 3:
        impl_->masm.Frintz(d_t, d_n);
        break;
    }
  }
...
void Emitter::vextend(FpReg rd, FpReg rn, VecLane narrow_lane, VecLane wide_lane, bool is_signed) {
  ...
  auto step = [&](int dst_v_code, int src_v_code, VecLane narrow) {
    ...
    if (narrow == VecLane::B16) {
      ...
      if (is_signed)
        impl_->masm.Sxtl(vd, vs);
      else
        impl_->masm.Uxtl(vd, vs);
  ...
  while (cur_narrow != wide_lane) {
    const VecLane next = (cur_narrow == VecLane::B16)  ? VecLane::H8
                         : (cur_narrow == VecLane::H8) ? VecLane::S4
                                                       : VecLane::D2;
    const int dst = (next == wide_lane) ? static_cast<int>(rd) : kInternalFpScratchV;
...
void Emitter::valignr(FpReg rd, FpReg lhs, FpReg rhs, std::uint8_t count) {
  ...
  if (count < 16) {
    ...
    impl_->masm.Ext(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(rhs), to_vixl_q_bitwise(lhs),
    ...
    if (count == 16) {
      ...
      impl_->masm.Ext(to_vixl_q_bitwise(rd), to_vixl_q_bitwise(lhs), v_z,
...
void Emitter::vfcmp_scalar_with_upper(FpReg rd, FpReg lhs, FpReg rhs, ir::FpSize sz,
                                      std::uint8_t pred) {
...
void Emitter::vmask_msb_b16(arm64::Reg w_dst, FpReg rn) {
  ...
  // 7. extract lo and hi to GPRs.
  {
    ...
    impl_->masm.Orr(to_vixl_w(w_dst), to_vixl_w(w_dst), vixl_aa::Operand(w_hi, vixl_aa::LSL, 8));
  }
}

void Emitter::vumov_w_from_lane(arm64::Reg w_dst, FpReg rn, std::uint8_t lane_idx, VecLane lane) {
  const auto fmt = vec_format_for_lane(lane);
...
void Emitter::vld1_q_offset(FpReg rd, arm64::Reg base, std::int32_t imm) {
  impl_->masm.Ldr(to_vixl_qreg(rd), vixl_aa::MemOperand(to_vixl_x(base), imm));
}
void Emitter::vst1_q_offset(FpReg rs, arm64::Reg base, std::int32_t imm) {
  impl_->masm.Str(to_vixl_qreg(rs), vixl_aa::MemOperand(to_vixl_x(base), imm));
}
...
void Emitter::dmb(BarrierKind k) {
  ...
  BarrierType bt;
  switch (k) {
    case BarrierKind::Ish:
      bt = vixl_aa::BarrierAll;
      break;
    case BarrierKind::IshLd:
      bt = vixl_aa::BarrierReads;
      break;
    case BarrierKind::IshSt:
      bt = vixl_aa::BarrierWrites;
      break;
  }
...
void Emitter::bind(Label label) {
  ...
  if (label.id == 0 || label.id > impl_->labels.size())
    return;
...
std::string Emitter::disassemble() const {
  ...
  for (const vixl_aa::Instruction* inst = start; inst < end; inst = inst->GetNextInstruction()) {
    decoder.Decode(inst);
    os << disasm.GetOutput() << "\n";
  }
  return os.str();
