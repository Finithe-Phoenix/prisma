...
bool is_block_terminator(const ir::Op& op) noexcept {
  return std::holds_alternative<ir::Return>(op) || std::holds_alternative<ir::JumpRel>(op) ||
  ...
  return std::visit(
      [](const auto& op) -> ExitMetadata {
        ...
        if constexpr (std::is_same_v<T, ir::Return>) {
          ...
          return {BlockExitKind::CondJumpRel, op.target_guest_pc, op.fallthrough_guest_pc, 0};
        } else if constexpr (std::is_same_v<T, ir::CallRel>) {
          return {BlockExitKind::CallRel, op.target_guest_pc, 0, op.return_guest_pc};
...
bool op_may_write_guest_memory(const ir::Op& op) noexcept {
  return std::visit(
      [](const auto& inner) noexcept {
        using T = std::decay_t<decltype(inner)>;
        return std::is_same_v<T, ir::StoreMem> || std::is_same_v<T, ir::StoreMemTSO> ||
               std::is_same_v<T, ir::AtomicCmpxchg> || std::is_same_v<T, ir::AtomicCmpxchgPair> ||
               std::is_same_v<T, ir::StoreVec> || std::is_same_v<T, ir::CallRel> ||
               std::is_same_v<T, ir::CallReg> || std::is_same_v<T, ir::RepStos> ||
               std::is_same_v<T, ir::RepMovs> || std::is_same_v<T, ir::InlineAsm>;
      },
      op);
}

bool body_may_write_guest_memory(const std::vector<ir::Stmt>& body) noexcept {
...
decode_until_terminator(std::span<const std::uint8_t> bytes, std::uint64_t guest_addr,
                        bool real_call_ret) {
  Decoded out;
  ir::Ref next = 0;
  std::size_t cursor = 0;
  ...
  while (cursor < bytes.size()) {
    ...
    auto res = decoder::decode_one(bytes.subspan(cursor), next, instr_pc, real_call_ret);
...
DirectPatchResult Translator::patch_direct_exit(std::uint64_t source_guest_pc,
  ...
  if (has_incoming_direct_patch(source_guest_pc) || target.direct_patch_applied) {
    return DirectPatchResult::WouldCreateChain;
  }

  const auto patched = pool_->patch_aarch64_branch(source.jit_block,
                                                   source.direct_patch.branch_offset, target.entry);
...
DirectPatchResult Translator::unpatch_direct_exit(std::uint64_t source_guest_pc) {
  auto source_it = by_addr_.find(source_guest_pc);
  if (source_it == by_addr_.end()) {
    return DirectPatchResult::SourceMissing;
  }
  ...
  const auto patched =
      pool_->patch_aarch64_branch(source.jit_block, source.direct_patch.branch_offset, fallback);
  if (patched != runtime::JitPatchResult::Patched) {
    return DirectPatchResult::PatchFailed;
  }
...
bool Translator::direct_exit_is_patched(std::uint64_t source_guest_pc) const {
  auto it = by_addr_.find(source_guest_pc);
  return it != by_addr_.end() && it->second.direct_patch_applied;
}

std::optional<TranslatedBlock>
Translator::active_direct_patch_target(std::uint64_t source_guest_pc) const {
  auto source_it = by_addr_.find(source_guest_pc);
  if (source_it == by_addr_.end()) {
    return std::nullopt;
  }
  ...
  return TranslatedBlock{
      rec.entry,           rec.code_size,   rec.guest_size,
      /*from_cache=*/true, rec.exit_kind,   rec.target_guest_pc, rec.fallthrough_guest_pc,
      rec.return_guest_pc, rec.direct_patch};
}
...
bool Translator::has_incoming_direct_patch(std::uint64_t target_guest_pc) const {
  auto incoming = direct_patch_incoming_.find(target_guest_pc);
  return incoming != direct_patch_incoming_.end() && !incoming->second.empty();
}

std::optional<TranslatedBlock>
Translator::lookup_cached(std::uint64_t guest_addr,
...
TranslateResult Translator::translate(std::uint64_t guest_addr,
  ...
  auto decoded = decode_until_terminator(guest_bytes, guest_addr, real_call_ret_);
  ...
  const bool body_has_terminator = !body.empty() && is_block_terminator(body.back().op);
  ...
  // decoded).
  lopts.cpuid_leaf1_edx = (1u << 0) | (1u << 4) | (1u << 8) | (1u << 15) | (1u << 25) | (1u << 26);
  ...
  lopts.cpuid_leaf1_ecx = (1u << 0) | (1u << 1) | (1u << 9) | (1u << 12) | (1u << 13) | (1u << 19) |
                          (1u << 20) | (1u << 22) | (1u << 23) | (1u << 27) | (1u << 28) |
                          (1u << 29);
  ...
  if (body_has_terminator && can_use_patchable_tail(exit.kind)) {
    ...
    direct_patch = TranslatedBlock::DirectPatchSite{
        /*available=*/true,
        /*auto_patch_safe=*/!body_may_write_guest_memory(body), patch.branch_offset,
        patch.fallback_offset, exit.target_guest_pc};
  } else {
  ...
  by_addr_[guest_addr] = Record{entry,
                                blk,
                                code_size,
                                dec.consumed,
                                hash_of_input,
