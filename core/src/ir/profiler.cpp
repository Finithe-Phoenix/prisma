...
OpCounter::Kind kind_for(const Op& op) noexcept {
    return std::visit([](const auto& x) -> OpCounter::Kind {
        ...
        else if constexpr (std::is_same_v<T, LoadCarry>)   return OpCounter::Kind::LoadCarry;
        else if constexpr (std::is_same_v<T, StoreCarry>)  return OpCounter::Kind::StoreCarry;
        else if constexpr (std::is_same_v<T, LoadRflags>)  return OpCounter::Kind::LoadRflags;
        else if constexpr (std::is_same_v<T, StoreRflags>) return OpCounter::Kind::StoreRflags;
        else if constexpr (std::is_same_v<T, StoreRflagsFromNzcv>) return OpCounter::Kind::StoreRflagsFromNzcv;
        else if constexpr (std::is_same_v<T, StoreRflagsFromBits>) return OpCounter::Kind::StoreRflagsFromBits;
        else if constexpr (std::is_same_v<T, BinOp>)       return OpCounter::Kind::BinOp;
        else if constexpr (std::is_same_v<T, WideDiv>)     return OpCounter::Kind::WideDiv;
        ...
        else if constexpr (std::is_same_v<T, AtomicCmpxchg>) return OpCounter::Kind::AtomicCmpxchg;
        else if constexpr (std::is_same_v<T, AtomicCmpxchgPair>) return OpCounter::Kind::AtomicCmpxchgPair;
        ...
        else if constexpr (std::is_same_v<T, TrapIf>)      return OpCounter::Kind::TrapIf;
        ...
        else if constexpr (std::is_same_v<T, VecClMul>)      return OpCounter::Kind::VecClMul;
        else if constexpr (std::is_same_v<T, VecF16Cvt>)     return OpCounter::Kind::VecF16Cvt;
        ...
        else if constexpr (std::is_same_v<T, PcmpStrIndex>)  return OpCounter::Kind::PcmpStrIndex;
        else if constexpr (std::is_same_v<T, PcmpStrMask>)   return OpCounter::Kind::PcmpStrMask;
        else if constexpr (std::is_same_v<T, PcmpStrFlags>)  return OpCounter::Kind::PcmpStrFlags;
        ...
        else if constexpr (std::is_same_v<T, VecFpRound>)    return OpCounter::Kind::VecFpRound;
        else if constexpr (std::is_same_v<T, Popcnt>)        return OpCounter::Kind::Popcnt;
        else if constexpr (std::is_same_v<T, Lzcnt>)         return OpCounter::Kind::Lzcnt;
        else if constexpr (std::is_same_v<T, Tzcnt>)         return OpCounter::Kind::Tzcnt;
        else if constexpr (std::is_same_v<T, WriteFlagsCountZero>) return OpCounter::Kind::WriteFlagsCountZero;
        ...
        else if constexpr (std::is_same_v<T, WriteFlagsPtestYmm>) return OpCounter::Kind::WriteFlagsPtestYmm;
        else if constexpr (std::is_same_v<T, VecTbl2>)       return OpCounter::Kind::VecTbl2;
        else if constexpr (std::is_same_v<T, VecAes>)        return OpCounter::Kind::VecAes;
        else if constexpr (std::is_same_v<T, Bswap>)         return OpCounter::Kind::Bswap;
        else if constexpr (std::is_same_v<T, Crc32c>)        return OpCounter::Kind::Crc32c;
        else if constexpr (std::is_same_v<T, VecGather>)     return OpCounter::Kind::VecGather;
        ...
        else if constexpr (std::is_same_v<T, VecFpFma>)      return OpCounter::Kind::VecFpFma;
