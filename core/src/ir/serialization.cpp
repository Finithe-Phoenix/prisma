...
class Writer {
 public:
  ...
  void root_op(const Op& op) { this->op(op); }
...
[[nodiscard]] bool decode_gpr(std::uint8_t raw, Gpr& out) {
  if (raw > static_cast<std::uint8_t>(Gpr::R15))
    return false;
...
[[nodiscard]] bool decode_size(std::uint8_t raw, OpSize& out) {
  if (raw > static_cast<std::uint8_t>(OpSize::I64))
    return false;
...
[[nodiscard]] bool decode_binop(std::uint8_t raw, BinOpKind& out) {
  if (raw > static_cast<std::uint8_t>(BinOpKind::Rcr))
    return false;
...
[[nodiscard]] bool decode_cond(std::uint8_t raw, CondCode& out) {
  if (raw > static_cast<std::uint8_t>(CondCode::Pl))
    return false;
...
[[nodiscard]] bool decode_segment(std::uint8_t raw, SegmentReg& out) {
  if (raw > static_cast<std::uint8_t>(SegmentReg::Gs))
    return false;
...
[[nodiscard]] bool decode_trap(std::uint8_t raw, TrapKind& out) {
  if (raw > static_cast<std::uint8_t>(TrapKind::Sigfpe))
    return false;
...
[[nodiscard]] bool decode_fence(std::uint8_t raw, FenceKind& out) {
  if (raw > static_cast<std::uint8_t>(FenceKind::Sfence))
    return false;
...
[[nodiscard]] bool decode_rflags_carry(std::uint8_t raw, RflagsCarryMode& out) {
  if (raw > static_cast<std::uint8_t>(RflagsCarryMode::Preserve))
    return false;
...
[[nodiscard]] bool decode_wide_div_result(std::uint8_t raw, WideDivResult& out) {
  if (raw > static_cast<std::uint8_t>(WideDivResult::Remainder))
    return false;
...
[[nodiscard]] bool decode_vec_f16cvt_kind(std::uint8_t raw, VecF16CvtKind& out) {
  if (raw > static_cast<std::uint8_t>(VecF16CvtKind::PsToPh))
    return false;
...
[[nodiscard]] std::variant<E, IrDeserializeError> read_enum(Reader& reader, Decode decode) {
  ...
  E value{};
  if (!decode(raw, value))
    return IrDeserializeError::InvalidEnum;
  return value;
}
...
[[nodiscard]] std::variant<bool, IrDeserializeError> read_bool(Reader& reader) {
  ...
  if (!reader.u8(raw))
    return reader.error();
  if (raw > 1u)
    return IrDeserializeError::InvalidBool;
  return raw == 1u;
}
  ...
  switch (tag) {
    ...
    case OpTag::StoreRflagsFromBits: {
      ...
      return Op{StoreRflagsFromBits{std::get<std::optional<Ref>>(pf),
                                    std::get<std::optional<Ref>>(af), zf, sf, of}};
    }
    case OpTag::StoreReg: {
      auto reg = read_enum<Gpr>(reader, decode_gpr);
      if (std::holds_alternative<IrDeserializeError>(reg)) {
    ...
    case OpTag::BinOp: {
      ...
      std::uint32_t lhs = 0;
      std::uint32_t rhs = 0;
      if (!reader.u32(lhs) || !reader.u32(rhs))
        return reader.error();
      auto size = read_enum<OpSize>(reader, decode_size);
      if (std::holds_alternative<IrDeserializeError>(size)) {
        return std::get<IrDeserializeError>(size);
    ...
    case OpTag::WideDiv: {
      ...
      return Op{
          WideDiv{high, low, divisor, std::get<bool>(is_signed), std::get<WideDivResult>(result)}};
    }
    case OpTag::PcmpStrIndex: {
      ...
      if (!reader.u8(imm8))
        return reader.error();
      return Op{PcmpStrIndex{lhs, rhs, std::get<std::optional<Ref>>(lhs_len),
                             std::get<std::optional<Ref>>(rhs_len), imm8}};
    }
    case OpTag::PcmpStrMask: {
      ...
      if (!reader.u8(imm8))
        return reader.error();
      return Op{PcmpStrMask{lhs, rhs, std::get<std::optional<Ref>>(lhs_len),
                            std::get<std::optional<Ref>>(rhs_len), imm8}};
    }
    case OpTag::PcmpStrFlags: {
      ...
      if (!reader.u8(imm8))
        return reader.error();
      return Op{PcmpStrFlags{lhs, rhs, std::get<std::optional<Ref>>(lhs_len),
                             std::get<std::optional<Ref>>(rhs_len), imm8}};
    }
    case OpTag::VecClMul: {
      Ref lhs{};
      ...
      return Op{VecClMul{lhs, rhs, std::get<bool>(lhs_high), std::get<bool>(rhs_high)}};
    }
    ...
    case OpTag::Extend: {
      ...
      return Op{Extend{value, std::get<OpSize>(from_size), std::get<OpSize>(to_size),
                       std::get<bool>(is_signed)}};
    }
    case OpTag::Truncate: {
      std::uint32_t value = 0;
      if (!reader.u32(value))
    ...
    case OpTag::Select: {
      ...
      if (!reader.u32(true_value) || !reader.u32(false_value))
        return reader.error();
      auto size = read_enum<OpSize>(reader, decode_size);
      if (std::holds_alternative<IrDeserializeError>(size)) {
        return std::get<IrDeserializeError>(size);
      }
      return Op{Select{std::get<CondCode>(cc), true_value, false_value, std::get<OpSize>(size)}};
    }
    case OpTag::LoadMem: {
      std::uint32_t addr = 0;
      if (!reader.u32(addr))
    ...
    case OpTag::StoreMem: {
      std::uint32_t addr = 0;
      std::uint32_t value = 0;
      if (!reader.u32(addr) || !reader.u32(value))
        return reader.error();
      auto size = read_enum<OpSize>(reader, decode_size);
      if (std::holds_alternative<IrDeserializeError>(size)) {
        return std::get<IrDeserializeError>(size);
    ...
    case OpTag::AtomicCmpxchg: {
      ...
      Ref expected{};
      Ref new_value{};
      if (!reader.u32(addr) || !reader.u32(expected) || !reader.u32(new_value)) {
        return reader.error();
      }
      auto size = read_enum<OpSize>(reader, decode_size);
      if (std::holds_alternative<IrDeserializeError>(size)) {
      ...
      return Op{AtomicCmpxchg{addr, expected, new_value, std::get<OpSize>(size)}};
    }
    case OpTag::AtomicCmpxchgPair: {
      ...
      Ref new_high{};
      Ref old_high{};
      if (!reader.u32(addr) || !reader.u32(expected_low) || !reader.u32(expected_high) ||
          !reader.u32(new_low) || !reader.u32(new_high) || !reader.u32(old_high)) {
        return reader.error();
      }
      return Op{AtomicCmpxchgPair{addr, expected_low, expected_high, new_low, new_high, old_high}};
    }
    case OpTag::GuestPc: {
      std::uint64_t pc = 0;
      if (!reader.u64(pc))
    ...
    case OpTag::CondJumpFlags: {
      ...
      std::uint32_t if_true = 0;
      std::uint32_t if_false = 0;
      if (!reader.u32(if_true) || !reader.u32(if_false))
        return reader.error();
    ...
    case OpTag::AluFlags: {
      ...
      return Op{AluFlags{std::get<BinOpKind>(op), lhs, rhs, std::get<OpSize>(size)}};
    }
    case OpTag::WriteFlagsCountZero: {
      std::uint32_t src = 0;
      std::uint32_t result = 0;
      if (!reader.u32(src) || !reader.u32(result))
        return reader.error();
      auto size = read_enum<OpSize>(reader, decode_size);
      if (std::holds_alternative<IrDeserializeError>(size)) {
        return std::get<IrDeserializeError>(size);
    ...
    case OpTag::CondJumpRel: {
      ...
      return Op{CondJumpRel{std::get<CondCode>(cc), target_guest_pc, fallthrough_guest_pc}};
    }
  }
  return IrDeserializeError::InvalidTag;
}
...
read_stmt_list_payload(Reader& reader) {
  ...
  if (!reader.u32(count))
    return reader.error();
  if (count > kMaxDeserializedItems)
    return IrDeserializeError::TooLarge;
  if (count > reader.remaining() / 2u)
    return IrDeserializeError::Truncated;
...
[[nodiscard]] std::optional<IrDeserializeError> read_header(Reader& reader,
  ...
  for (const auto expected : kIrBinaryMagic) {
    ...
    if (!reader.u8(actual))
      return reader.error();
    if (actual != expected)
      return IrDeserializeError::BadMagic;
  }

  std::uint16_t version = 0;
  if (!reader.u16(version))
    return reader.error();
  if (version != kIrBinaryVersion)
    return IrDeserializeError::UnsupportedVersion;
...
[[nodiscard]] std::optional<IrDeserializeError> check_trailing(const Reader& reader) {
  if (!reader.empty())
    return IrDeserializeError::TrailingBytes;
  return std::nullopt;
}
...
deserialize_binary_stmts(std::span<const std::uint8_t> bytes) {
  Reader reader(bytes);
  if (auto error = read_header(reader, IrBinaryKind::StmtList))
    return *error;
  ...
  if (auto error = check_trailing(reader))
    return *error;
  return std::get<std::vector<Stmt>>(std::move(stmts));
}

std::variant<Op, IrDeserializeError> deserialize_binary_op(std::span<const std::uint8_t> bytes) {
  Reader reader(bytes);
  auto op = read_op(reader);
  if (std::holds_alternative<IrDeserializeError>(op)) {
    return std::get<IrDeserializeError>(op);
...
deserialize_binary_function(std::span<const std::uint8_t> bytes) {
  Reader reader(bytes);
  if (auto error = read_header(reader, IrBinaryKind::Function))
    return *error;

  Function function;
  if (!reader.u32(function.entry))
    return reader.error();

  std::uint32_t block_count = 0;
  if (!reader.u32(block_count))
    return reader.error();
  if (block_count > kMaxDeserializedItems)
    return IrDeserializeError::TooLarge;
