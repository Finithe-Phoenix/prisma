...
constexpr std::uint8_t kMaxOpKind = static_cast<std::uint8_t>(OpKind::kVecF16Cvt);
...
struct Cursor {
  ...
  [[nodiscard]] bool remaining(std::size_t n) const noexcept { return pos + n <= data.size(); }

  [[nodiscard]] std::uint8_t take_u8() noexcept { return data[pos++]; }
...
void write_payload(std::vector<std::uint8_t>& out, const VecFpFma& x) {
  ...
  const std::uint8_t flags =
      static_cast<std::uint8_t>((x.neg_addend ? 0x01u : 0u) | (x.neg_mul ? 0x02u : 0u));
  put_u8(out, flags);
  put_u8(out, static_cast<std::uint8_t>(x.size));
...
void write_pcmpstr_payload(std::vector<std::uint8_t>& out, Ref lhs, Ref rhs,
                           std::optional<Ref> lhs_len, std::optional<Ref> rhs_len,
                           std::uint8_t imm8) {
  put_u32(out, lhs);
  put_u32(out, rhs);
  const std::uint8_t flags =
      static_cast<std::uint8_t>((lhs_len ? 0x01u : 0u) | (rhs_len ? 0x02u : 0u));
  put_u8(out, flags);
  if (lhs_len)
    put_u32(out, *lhs_len);
  if (rhs_len)
...
DeserializeError read_payload_store_rflags_from_nzcv(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_rflags_carry(carry))
    return DeserializeError::BadSize;
  std::optional<Ref> pf;
  auto err = take_optional_ref(c, pf);
  if (err != DeserializeError::Ok)
    return err;
  std::optional<Ref> af;
...
DeserializeError read_payload_wide_div(Cursor& c, Stmt& s) {
  ...
  if (is_signed > 1u)
    return DeserializeError::BadSize;
  if (!is_valid_wide_div_result(result))
    return DeserializeError::BadSize;
  s.op = WideDiv{high, low, divisor, is_signed == 1u, static_cast<WideDivResult>(result)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_extend(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_size(fs) || !is_valid_size(ts))
    return DeserializeError::BadSize;
  if (sg > 1u)
    return DeserializeError::BadSize;
  s.op = Extend{v, static_cast<OpSize>(fs), static_cast<OpSize>(ts), sg == 1u};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_fp_binop(Cursor& c, Stmt& s) {
  ...
  const std::uint32_t rhs = c.take_u32();
  const std::uint8_t sz = c.take_u8();
  if (!is_valid_fpbinop(op))
    return DeserializeError::BadSize;
  if (!is_valid_fpsize(sz))
    return DeserializeError::BadSize;
  s.op = FpBinOp{static_cast<FpBinOpKind>(op), lhs, rhs, static_cast<FpSize>(sz)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_write_flags(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_size(sz))
    return DeserializeError::BadSize;
  s.op = WriteFlags{static_cast<BinOpKind>(op), lhs, rhs, static_cast<OpSize>(sz)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_alu_flags(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_size(sz))
    return DeserializeError::BadSize;
  s.op = AluFlags{static_cast<BinOpKind>(op), lhs, rhs, static_cast<OpSize>(sz)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_vec_binop(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_vecbinop(op))
    return DeserializeError::BadSize;
  if (!is_valid_veclane(lane))
    return DeserializeError::BadSize;
  s.op = VecBinOp{static_cast<VecBinOpKind>(op), lhs, rhs, static_cast<VecLane>(lane)};
  return DeserializeError::Ok;
}

DeserializeError read_payload_vec_clmul(Cursor& c, Stmt& s) {
  if (!c.remaining(4 + 4 + 1 + 1))
    return DeserializeError::Truncated;
  const std::uint32_t lhs = c.take_u32();
  const std::uint32_t rhs = c.take_u32();
  const std::uint8_t lhs_high = c.take_u8();
  const std::uint8_t rhs_high = c.take_u8();
  if (lhs_high > 1u || rhs_high > 1u)
    return DeserializeError::BadSize;
...
DeserializeError read_payload_vec_fp_scalar_fma(Cursor& c, Stmt& s) {
  ...
  (void)pad;
  if (!is_valid_fpsize(size))
    return DeserializeError::BadSize;
  s.op = VecFpScalarFma{
      a, b, cc, up, (flags & 0x01u) != 0, (flags & 0x02u) != 0, static_cast<FpSize>(size)};
  return DeserializeError::Ok;
}

DeserializeError read_payload_rep_stos(Cursor& c, Stmt& s) {
  if (!c.remaining(2 + 8 + 8))
    return DeserializeError::Truncated;
  ...
  if (size_b > static_cast<std::uint8_t>(OpSize::I64))
    return DeserializeError::BadSize;
...
DeserializeError read_payload_vec_fp_binop(Cursor& c, Stmt& s) {
  ...
  const std::uint32_t rhs = c.take_u32();
  const std::uint8_t size = c.take_u8();
  if (!is_valid_vecfpbinop(op))
    return DeserializeError::BadSize;
  if (!is_valid_vecfpsize(size))
    return DeserializeError::BadSize;
  s.op = VecFpBinOp{static_cast<VecFpBinOpKind>(op), lhs, rhs, static_cast<VecFpSize>(size)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_vec_gather(Cursor& c, Stmt& s) {
  ...
  if (scale > 3u || e64 > 1u || i64 > 1u)
    return DeserializeError::BadSize;
  if (lanes != 2u && lanes != 4u)
    return DeserializeError::BadSize;
  // Lane windows must fit the 128-bit half at their own widths.
  if (dbase + lanes > (e64 ? 2u : 4u))
    return DeserializeError::BadSize;
  if (ibase + lanes > (i64 ? 2u : 4u))
    return DeserializeError::BadSize;
  s.op = VecGather{base, index, mask, prev, scale, e64, i64, lanes, dbase, ibase};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_vec_extend(Cursor& c, Stmt& s) {
  ...
  if (is_s > 1)
    return DeserializeError::BadSize;
  s.op = VecExtend{src, static_cast<VecLane>(narrow), static_cast<VecLane>(wide), is_s == 1};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_vec_fp_compare(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_fpsize(size))
    return DeserializeError::BadSize;
  if (pred > static_cast<std::uint8_t>(VecFpCmpPred::Ord))
    return DeserializeError::BadSize;
  if (packed > 1)
    return DeserializeError::BadSize;
  s.op = VecFpCompare{lhs, rhs, static_cast<FpSize>(size), static_cast<VecFpCmpPred>(pred),
                      packed == 1};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_fp_cvt_scalar(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_fpsize(ss) || !is_valid_fpsize(ds))
    return DeserializeError::BadSize;
...
DeserializeError read_payload_int_to_fp_scalar(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_size(isz))
    return DeserializeError::BadSize;
  if (!is_valid_fpsize(fsz))
    return DeserializeError::BadSize;
...
DeserializeError read_payload_vec_shift_imm(Cursor& c, Stmt& s) {
  if (!c.remaining(1 + 4 + 1 + 1))
    return DeserializeError::Truncated;
  const std::uint8_t kind = c.take_u8();
  const std::uint32_t src = c.take_u32();
  const std::uint8_t count = c.take_u8();
  const std::uint8_t lane = c.take_u8();
  if (kind > static_cast<std::uint8_t>(VecShiftKind::ArithShr))
    return DeserializeError::BadSize;
  if (!is_valid_veclane(lane))
    return DeserializeError::BadSize;
  s.op = VecShiftImm{static_cast<VecShiftKind>(kind), src, count, static_cast<VecLane>(lane)};
  return DeserializeError::Ok;
}
...
DeserializeError read_payload_vec_cmp(Cursor& c, Stmt& s) {
  ...
  const std::uint32_t rhs = c.take_u32();
  const std::uint8_t lane = c.take_u8();
  if (kind > static_cast<std::uint8_t>(VecCmpKind::Gt))
    return DeserializeError::BadSize;
  if (!is_valid_veclane(lane))
    return DeserializeError::BadSize;
  s.op = VecCmp{static_cast<VecCmpKind>(kind), lhs, rhs, static_cast<VecLane>(lane)};
  return DeserializeError::Ok;
}
...
DeserializeError read_pcmpstr_payload(Cursor& c, Ref& lhs, Ref& rhs, std::optional<Ref>& lhs_len,
                                      std::optional<Ref>& rhs_len, std::uint8_t& imm8) {
  if (!c.remaining(4 + 4 + 1))
    return DeserializeError::Truncated;
  lhs = c.take_u32();
  rhs = c.take_u32();
...
DeserializeError read_payload_vec_fp_scalar_binop(Cursor& c, Stmt& s) {
  ...
  if (!is_valid_fpsize(size))
    return DeserializeError::BadSize;
  s.op = VecFpScalarBinOp{static_cast<VecFpBinOpKind>(op), lhs, rhs, static_cast<FpSize>(size)};
  return DeserializeError::Ok;
}
...
DeserializeError read_stmt(Cursor& c, Stmt& s) {
  ...
  switch (static_cast<OpKind>(kind)) {
    case OpKind::kConstant:
      return read_payload_constant(c, s);
    case OpKind::kLoadReg:
      return read_payload_load_reg(c, s);
    case OpKind::kStoreReg:
      return read_payload_store_reg(c, s);
    case OpKind::kLoadSegBase:
      return read_payload_load_seg_base(c, s);
    case OpKind::kLoadCarry:
      return read_payload_load_carry(c, s);
    case OpKind::kStoreCarry:
      return read_payload_store_carry(c, s);
    case OpKind::kLoadRflags:
      return read_payload_load_rflags(c, s);
    case OpKind::kStoreRflags:
      return read_payload_store_rflags(c, s);
    case OpKind::kStoreRflagsFromNzcv:
      return read_payload_store_rflags_from_nzcv(c, s);
    case OpKind::kStoreRflagsFromBits:
      return read_payload_store_rflags_from_bits(c, s);
    case OpKind::kBinOp:
      return read_payload_binop(c, s);
    case OpKind::kWideDiv:
      return read_payload_wide_div(c, s);
    case OpKind::kCompare:
      return read_payload_compare(c, s);
    case OpKind::kSelect:
      return read_payload_select(c, s);
    case OpKind::kLoadMem:
      return read_payload_load_mem(c, s);
    case OpKind::kStoreMem:
      return read_payload_store_mem(c, s);
    case OpKind::kLoadMemTSO:
      return read_payload_load_mem_tso(c, s);
    case OpKind::kStoreMemTSO:
      return read_payload_store_mem_tso(c, s);
    case OpKind::kAtomicCmpxchg:
      return read_payload_atomic_cmpxchg(c, s);
    case OpKind::kAtomicCmpxchgPair:
      return read_payload_atomic_cmpxchg_pair(c, s);
    case OpKind::kJump:
      return read_payload_jump(c, s);
    case OpKind::kCondJump:
      return read_payload_cond_jump(c, s);
    case OpKind::kReturn:
      return read_payload_return(c, s);
    case OpKind::kJumpReg:
      return read_payload_jump_reg(c, s);
    case OpKind::kCmpFlags:
      return read_payload_cmp_flags(c, s);
    case OpKind::kAluFlags:
      return read_payload_alu_flags(c, s);
    case OpKind::kJumpRel:
      return read_payload_jump_rel(c, s);
    case OpKind::kCondJumpRel:
      return read_payload_cond_jump_rel(c, s);
    case OpKind::kCallRel:
      return read_payload_call_rel(c, s);
    case OpKind::kCallReg:
      return read_payload_call_reg(c, s);
    case OpKind::kRetAdjusted:
      return read_payload_ret_adjusted(c, s);
    case OpKind::kCpuid:
      return read_payload_cpuid(c, s);
    case OpKind::kSyscall:
      return read_payload_syscall(c, s);
    case OpKind::kTrap:
      return read_payload_trap(c, s);
    case OpKind::kTrapIf:
      return read_payload_trap_if(c, s);
    case OpKind::kExtend:
      return read_payload_extend(c, s);
    case OpKind::kTruncate:
      return read_payload_truncate(c, s);
    case OpKind::kFence:
      return read_payload_fence(c, s);
    case OpKind::kGuestPc:
      return read_payload_guest_pc(c, s);
    case OpKind::kInlineAsm:
      return read_payload_inline_asm(c, s);
    case OpKind::kFpConstant:
      return read_payload_fp_constant(c, s);
    case OpKind::kFpBinOp:
      return read_payload_fp_binop(c, s);
    case OpKind::kWriteFlags:
      return read_payload_write_flags(c, s);
    case OpKind::kReadFlag:
      return read_payload_read_flag(c, s);
    case OpKind::kCondJumpFlags:
      return read_payload_cond_jump_flags(c, s);
    case OpKind::kRspAdjust:
      return read_payload_rsp_adjust(c, s);
    case OpKind::kVecConstant:
      return read_payload_vec_constant(c, s);
    case OpKind::kVecBinOp:
      return read_payload_vec_binop(c, s);
    case OpKind::kVecClMul:
      return read_payload_vec_clmul(c, s);
    case OpKind::kVecF16Cvt:
      return read_payload_vec_f16cvt(c, s);
    case OpKind::kLoadVecReg:
      return read_payload_load_vec_reg(c, s);
    case OpKind::kStoreVecReg:
      return read_payload_store_vec_reg(c, s);
    case OpKind::kVecFpBinOp:
      return read_payload_vec_fp_binop(c, s);
    case OpKind::kVecFpScalarBinOp:
      return read_payload_vec_fp_scalar_binop(c, s);
    case OpKind::kLoadVec:
      return read_payload_load_vec(c, s);
    case OpKind::kStoreVec:
      return read_payload_store_vec(c, s);
    case OpKind::kPcmpStrIndex:
      return read_payload_pcmp_str_index(c, s);
    case OpKind::kPcmpStrMask:
      return read_payload_pcmp_str_mask(c, s);
    case OpKind::kPcmpStrFlags:
      return read_payload_pcmp_str_flags(c, s);
    case OpKind::kXmmFromGpr:
      return read_payload_xmm_from_gpr(c, s);
    case OpKind::kGprFromXmm:
      return read_payload_gpr_from_xmm(c, s);
    case OpKind::kVecCmp:
      return read_payload_vec_cmp(c, s);
    case OpKind::kVecShuffle32x4:
      return read_payload_vec_shuffle32x4(c, s);
    case OpKind::kVecUnpack:
      return read_payload_vec_unpack(c, s);
    case OpKind::kVecShiftImm:
      return read_payload_vec_shift_imm(c, s);
    case OpKind::kVecShiftBytes:
      return read_payload_vec_shift_bytes(c, s);
    case OpKind::kIntToFpScalar:
      return read_payload_int_to_fp_scalar(c, s);
    case OpKind::kFpToIntScalar:
      return read_payload_fp_to_int_scalar(c, s);
    case OpKind::kFpCvtScalar:
      return read_payload_fp_cvt_scalar(c, s);
    case OpKind::kVecShuffle2Src:
      return read_payload_vec_shuffle_2src(c, s);
    case OpKind::kVecInsertLane:
      return read_payload_vec_insert_lane(c, s);
    case OpKind::kVecExtractLaneU:
      return read_payload_vec_extract_lane_u(c, s);
    case OpKind::kVecMaskMsb:
      return read_payload_vec_mask_msb(c, s);
    case OpKind::kWriteFlagsFp:
      return read_payload_write_flags_fp(c, s);
    case OpKind::kVecShuffleH4:
      return read_payload_vec_shuffle_h4(c, s);
    case OpKind::kVecMaskFp:
      return read_payload_vec_mask_fp(c, s);
    case OpKind::kVecFpCompare:
      return read_payload_vec_fp_compare(c, s);
    case OpKind::kVecPshufb:
      return read_payload_vec_pshufb(c, s);
    case OpKind::kVecAbs:
      return read_payload_vec_abs(c, s);
    case OpKind::kVecAlignr:
      return read_payload_vec_alignr(c, s);
    case OpKind::kVecExtend:
      return read_payload_vec_extend(c, s);
    case OpKind::kVecFpRound:
      return read_payload_vec_fp_round(c, s);
    case OpKind::kPopcnt:
      return read_payload_popcnt(c, s);
    case OpKind::kLzcnt:
      return read_payload_lzcnt(c, s);
    case OpKind::kTzcnt:
      return read_payload_tzcnt(c, s);
    case OpKind::kWriteFlagsCountZero:
      return read_payload_write_flags_count_zero(c, s);
    case OpKind::kVecBlend:
      return read_payload_vec_blend(c, s);
    case OpKind::kWriteFlagsPtest:
      return read_payload_write_flags_ptest(c, s);
    case OpKind::kLoadVecRegHi:
      return read_payload_load_vec_reg_hi(c, s);
    case OpKind::kStoreVecRegHi:
      return read_payload_store_vec_reg_hi(c, s);
    case OpKind::kVecFpFma:
      return read_payload_vec_fp_fma(c, s);
    case OpKind::kVecFpScalarFma:
      return read_payload_vec_fp_scalar_fma(c, s);
    case OpKind::kRepStos:
      return read_payload_rep_stos(c, s);
    case OpKind::kRepMovs:
      return read_payload_rep_movs(c, s);
    case OpKind::kWriteFlagsPtestYmm:
      return read_payload_write_flags_ptest_ymm(c, s);
    case OpKind::kVecTbl2:
      return read_payload_vec_tbl2(c, s);
    case OpKind::kVecAes:
      return read_payload_vec_aes(c, s);
    case OpKind::kVecAesKeygenAssist:
      return read_payload_vec_aes_keygenassist(c, s);
    case OpKind::kVecSha:
      return read_payload_vec_sha(c, s);
    case OpKind::kBswap:
      return read_payload_bswap(c, s);
    case OpKind::kCrc32c:
      return read_payload_crc32c(c, s);
    case OpKind::kVecGather:
      return read_payload_vec_gather(c, s);
    case OpKind::kXgetbv:
      return read_payload_xgetbv(c, s);
    case OpKind::kRdtsc:
      return read_payload_rdtsc(c, s);
    case OpKind::kX87Load:
      return read_payload_x87_load(c, s);
    case OpKind::kX87Store:
      return read_payload_x87_store(c, s);
    case OpKind::kX87Push:
      return read_payload_x87_push(c, s);
    case OpKind::kX87Pop:
      return read_payload_x87_pop(c, s);
    case OpKind::kReserved:
      break;
  }
  return DeserializeError::UnknownOpKind;
}
...
DeserializeResult deserialize_stmts(std::span<const std::uint8_t> bytes) {
  ...
  const std::uint32_t got = static_cast<std::uint32_t>(bytes[crc_pos]) |
