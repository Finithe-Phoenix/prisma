  ...
  std::visit(
      [&](auto const& x) {
        ...
        if constexpr (std::is_same_v<T, ir::Constant>) {
          ...
          into.insert(x.rhs);
          if (x.lhs_len)
            into.insert(*x.lhs_len);
          if (x.rhs_len)
            into.insert(*x.rhs_len);
        } else if constexpr (std::is_same_v<T, ir::PcmpStrMask>) {
          into.insert(x.lhs);
          into.insert(x.rhs);
          if (x.lhs_len)
...
std::vector<ir::Stmt> dead_code_eliminate(const std::vector<ir::Stmt>& stmts) {
  ...
  for (const ir::Stmt& s : stmts) {
    const bool impure = !is_pure_for_dce(s.op);
    const bool result_is_live = s.result.has_value() && live.count(*s.result) != 0;
