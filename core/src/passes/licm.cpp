...
bool stmt_is_pure(const ir::Op& op) noexcept {
  return std::visit(
      [](auto const& x) -> bool {
        ...
        return std::is_same_v<T, ir::Constant> || std::is_same_v<T, ir::LoadReg> ||
               std::is_same_v<T, ir::LoadSegBase> || std::is_same_v<T, ir::BinOp> ||
               std::is_same_v<T, ir::WideDiv> || std::is_same_v<T, ir::Compare> ||
               std::is_same_v<T, ir::Extend> || std::is_same_v<T, ir::Truncate> ||
               std::is_same_v<T, ir::VecClMul> || std::is_same_v<T, ir::VecF16Cvt> ||
               std::is_same_v<T, ir::PcmpStrIndex> || std::is_same_v<T, ir::PcmpStrMask> ||
  ...
  std::visit(
      [&](auto const& x) {
        ...
        if constexpr (std::is_same_v<T, ir::BinOp>) {
          into.push_back(x.lhs);
          into.push_back(x.rhs);
        } else if constexpr (std::is_same_v<T, ir::WideDiv>) {
          into.push_back(x.high);
          into.push_back(x.low);
          into.push_back(x.divisor);
          ...
          into.push_back(x.rhs);
          if (x.lhs_len)
            into.push_back(*x.lhs_len);
          if (x.rhs_len)
            into.push_back(*x.rhs_len);
...
ir::Function loop_invariant_motion(const ir::Function& fn) {
  ...
  for (const auto& loop : loops) {
    ...
    std::unordered_set<std::uint32_t> body_ids(loop.body.begin(), loop.body.end());
      ...
      for (auto bid : loop.body) {
        ...
        while (i < terminator_pos) {
          ...
          if (ph_stmts.empty()) {
            ...
            ph_stmts.insert(ph_stmts.begin() + static_cast<std::ptrdiff_t>(ph_stmts.size() - 1),
                            std::move(hoisted));
          }
