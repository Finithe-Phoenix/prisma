...
std::uint16_t read_u16le(const std::vector<std::uint8_t>& bytes, std::size_t off) {
  ...
  const auto value =
      static_cast<std::uint32_t>(bytes[off]) | (static_cast<std::uint32_t>(bytes[off + 1u]) << 8u);
...
std::vector<Stmt> all_op_variant_stmts() {
  return {
      ...
      {3u, LoadCarry{}},
      {std::nullopt, StoreCarry{3u}},
      {4u, LoadRflags{}},
      {std::nullopt, StoreRflags{4u}},
      {std::nullopt, StoreRflagsFromNzcv{RflagsCarryMode::Preserve, 6u, std::nullopt}},
      {std::nullopt, StoreRflagsFromBits{6u, 7u, 8u, 9u, 10u}},
      {std::nullopt, StoreReg{Gpr::Rbx, 7u, OpSize::I32}},
      {5u, BinOp{BinOpKind::Add, 1u, 2u, OpSize::I64}},
      {38u, WideDiv{1u, 0u, 2u, false, WideDivResult::Quotient}},
      {39u, PcmpStrIndex{1u, 2u, std::nullopt, std::nullopt, 0x00u}},
      {40u, PcmpStrMask{1u, 2u, 3u, 4u, 0x48u}},
      {41u, PcmpStrFlags{1u, 2u, 3u, 4u, 0x08u}},
      {42u, VecClMul{1u, 2u, true, false}},
      {43u, VecF16Cvt{VecF16CvtKind::PhToPs, 1u, 0u}},
      {44u, VecF16Cvt{VecF16CvtKind::PsToPh, 2u, 0x0Bu}},
      ...
      {std::nullopt, TrapIf{0u, TrapKind::Sigfpe}},
