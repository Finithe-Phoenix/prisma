...
// Two AArch64 NOPs. Used as safely patchable placeholder bytes.
constexpr std::uint8_t kTwoNops[] = {
...
TEST_CASE("JitBufferPool: many small acquires fit in a single slab") {
  ...
  for (int i = 0; i < 1000; ++i) {
    blocks.push_back(pool.acquire(std::span<const std::uint8_t>(kRetInstruction)));
  }
  auto s = pool.stats();
...
TEST_CASE("JitBufferPool: patches an AArch64 branch forward in-place") {
  runtime::JitSlabPool pool;
  auto blk = pool.acquire(std::span<const std::uint8_t>(kTwoNops));

  REQUIRE(pool.patch_aarch64_branch(blk, 0, blk.entry + 4) == runtime::JitPatchResult::Patched);

  REQUIRE(read_u32(blk.entry) == 0x14000001u);      // b +4
  REQUIRE(read_u32(blk.entry + 4) == 0xD503201Fu);  // second NOP unchanged
}

TEST_CASE("JitBufferPool: patches an AArch64 branch backward in-place") {
  runtime::JitSlabPool pool;
  auto blk = pool.acquire(std::span<const std::uint8_t>(kTwoNops));

  REQUIRE(pool.patch_aarch64_branch(blk, 4, blk.entry) == runtime::JitPatchResult::Patched);

  REQUIRE(read_u32(blk.entry) == 0xD503201Fu);      // first NOP unchanged
  REQUIRE(read_u32(blk.entry + 4) == 0x17FFFFFFu);  // b -4
}

TEST_CASE("JitBufferPool: rejects unsafe AArch64 branch patches") {
  runtime::JitSlabPool pool;
  auto blk = pool.acquire(std::span<const std::uint8_t>(kTwoNops));

  runtime::JitBlock foreign = blk;
  foreign.slab_index = 999u;
  REQUIRE(pool.patch_aarch64_branch(foreign, 0, blk.entry) ==
          runtime::JitPatchResult::SiteNotOwned);

  REQUIRE(pool.patch_aarch64_branch(blk, blk.size_bytes, blk.entry) ==
          runtime::JitPatchResult::SiteNotOwned);

  REQUIRE(pool.patch_aarch64_branch(blk, 1, blk.entry) == runtime::JitPatchResult::Unaligned);
  REQUIRE(pool.patch_aarch64_branch(blk, 0, blk.entry + 1) == runtime::JitPatchResult::Unaligned);

  const auto far_addr = reinterpret_cast<std::uintptr_t>(blk.entry) + (128ULL << 20);
  const auto* far_target = reinterpret_cast<const std::uint8_t*>(far_addr);
  REQUIRE(pool.patch_aarch64_branch(blk, 0, far_target) == runtime::JitPatchResult::OutOfRange);
}
