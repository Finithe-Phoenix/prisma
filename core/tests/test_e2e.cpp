...
std::vector<ir::Stmt> decode_all(std::span<const std::uint8_t> guest_bytes) {
  ...
  while (cursor < guest_bytes.size()) {
    auto res = decoder::decode_one(guest_bytes.subspan(cursor), next);
...
TEST_CASE("e2e executing: translate and run `MOV rax, 42 ; RET`", "[arm64-only]") {
...
TEST_CASE(
    "e2e executing: translate and run `MOV rax, 100 ; MOV rbx, 42 ; XOR rax, rbx ; RET` (= 78)",
    "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  ...
  //   C3                               ret
  const std::vector<std::uint8_t> guest_bytes = {
      0x48, 0xB8, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0xBB,
      0x2A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0x31, 0xD8, 0xC3,
  };
  auto stmts = decode_all(guest_bytes);

  backend::Emitter em;
  backend::abi::emit_block_prologue(em);
  backend::LowerOptions opts{/*emit_ret_on_terminator=*/false};
  ...
  Fn fn = reinterpret_cast<Fn>(const_cast<std::uint8_t*>(jit.entry()));
  (void)fn(&state);
  REQUIRE(state.gpr[static_cast<std::size_t>(ir::Gpr::Rax)] == (100u ^ 42u));  // 78
}
...
TEST_CASE("e2e executing: load from guest memory returns stored value", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: store then re-load round-trips through guest memory", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: CMP + JNE is symmetric to CMP + JE", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: unconditional JumpRel returns its target", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: Translator end-to-end for decoded CMP + JE sequence", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
  ...
  const std::vector<std::uint8_t> guest_bytes = {
      0x48, 0xB8, 0x2A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0xBB, 0x2A,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0x39, 0xD8, 0x74, 0x10,
  };
...
TEST_CASE("e2e executing: decoded x86 `mov rax, [rbx]` JITs and runs", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: i8/i16/i32/i64 loads each see the right width", "[arm64-only]") {
  if constexpr (!is_arm64) {
    SUCCEED("skipped on non-ARM64 host");
    return;
  }
...
TEST_CASE("e2e executing: passes fold constants — output matches unoptimised", "[arm64-only]") {
  ...
  auto run = [&](std::vector<ir::Stmt> stmts) {
    ...
    Fn fn = reinterpret_cast<Fn>(const_cast<std::uint8_t*>(jit.entry()));
    (void)fn(&state);
    return state.gpr[static_cast<std::size_t>(ir::Gpr::Rax)];
  };
