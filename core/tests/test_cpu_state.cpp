...
TEST_CASE("CpuStateFrame: xmm[] is 16 entries × 16 bytes (256 bytes total)") {
  REQUIRE(sizeof(runtime::CpuStateFrame::xmm) == runtime::kXmmCount * sizeof(runtime::XmmReg));
...
TEST_CASE("CpuStateFrame: ymm_hi array occupies 256 bytes") {
  REQUIRE(sizeof(runtime::CpuStateFrame::ymm_hi) == runtime::kYmmCount * sizeof(runtime::XmmReg));
