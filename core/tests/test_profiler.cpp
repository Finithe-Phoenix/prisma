...
TEST_CASE("OpCounter: visit a Function rolls every block in") {
  ...
  Function fn;
  fn.entry = 0;
  fn.blocks.push_back(BasicBlock{0u, {{0u, Constant{1, OpSize::I64}}, {std::nullopt, Jump{1u}}}});
  fn.blocks.push_back(BasicBlock{1u, {{1u, Constant{2, OpSize::I64}}, {std::nullopt, Return{}}}});
  c.visit(fn);
  REQUIRE(c.count(OpCounter::Kind::Constant) == 2);
...
TEST_CASE("OpCounter: Kind covers every Op variant") {
  ...
  c.visit(Stmt{std::nullopt, StoreRflagsFromNzcv{RflagsCarryMode::ArmCarry, 63u, 64u}});
  c.visit(Stmt{std::nullopt, StoreRflagsFromBits{63u, 64u, 4u, 5u, 6u}});
  ...
  c.visit(Stmt{std::nullopt, CondJumpRel{CondCode::Eq, 0x100ULL, 0x200ULL}});
  ...
  c.visit(Stmt{8u, Extend{1u, OpSize::I8, OpSize::I64, /*signed=*/true}});
  ...
  c.visit(Stmt{17u, VecFpBinOp{VecFpBinOpKind::Add, 14u, 14u, VecFpSize::S4}});
  c.visit(Stmt{18u, VecFpScalarBinOp{VecFpBinOpKind::Add, 14u, 14u, FpSize::F32}});
  ...
  c.visit(Stmt{37u, VecFpCompare{14u, 14u, FpSize::F32, VecFpCmpPred::Eq, true}});
  ...
  REQUIRE(c.total() == static_cast<std::uint64_t>(OpCounter::Kind::kCount));
}
