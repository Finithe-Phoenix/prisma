...
runtime::CpuStateFrame run_blob(std::vector<std::uint8_t> bytes) {
  ...
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    ...
    if (off >= bytes.size())
      return {};
    return std::span<const std::uint8_t>(bytes.data() + off, bytes.size() - off);
  };
...
TEST_CASE("e2e: chained ALU — (5+3)*2 = 16 via add then add (no MUL yet)") {
  ...
  auto state = run_blob({
      0x48, 0xB8, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0xB9,
      0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x48, 0x01, 0xC8,  // add rax, rcx → 8
...
TEST_CASE("e2e: stats — single-block program reports blocks_executed = 1") {
  ...
  std::vector<std::uint8_t> bytes{
      0x48, 0xB8, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
      return {};
...
TEST_CASE("e2e: VADDPS xmm2, xmm0, xmm1 — AVX-128 3-operand packed FP add") {
  ...
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    ...
    if (off >= code.size())
      return {};
    return std::span<const std::uint8_t>(code.data() + off, code.size() - off);
  };
  runtime::Dispatcher disp{tx, reader};
  disp.install_halt_return_stack();
...
TEST_CASE("e2e: XADD sets carry flag for a following JC") {
  ...
  auto state = run_blob({
      ...
      0x48, 0xB9, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // mov rcx, 1
...
TEST_CASE("e2e: PMOVZXBW — F2-IR-041 zero-extend 8 bytes to 8 halfwords") {
  ...
  auto pack4h = [](std::uint16_t a, std::uint16_t b, std::uint16_t c,
                   std::uint16_t d) -> std::uint64_t {
    return static_cast<std::uint64_t>(a) | (static_cast<std::uint64_t>(b) << 16) |
           (static_cast<std::uint64_t>(c) << 32) | (static_cast<std::uint64_t>(d) << 48);
  };
...
TEST_CASE("e2e: PSHUFB byte-permute with MSB-set zero gating (SSSE3)") {
  ...
  std::uint8_t exp_lo[8] = {src_hi[7], src_lo[0], src_lo[7], 0, src_lo[1], src_hi[6], 0, src_lo[0]};
...
TEST_CASE("e2e: PSADBW — F2-IR-031 sum of byte differences (used by memcmp)") {
  ...
  disp.state().xmm[0].lo =
      0x5040302010ULL | (0x80706050ULL << 32);   // bytes 10,20,30,40,50,60,70,80
...
TEST_CASE("e2e: PSHUFLW reverses low 4 words, high half passes through") {
  ...
  runtime::Dispatcher disp{tx, reader};
  disp.install_halt_return_stack();
  auto pack4 = [](std::uint16_t a, std::uint16_t b, std::uint16_t c,
                  std::uint16_t d) -> std::uint64_t {
    return static_cast<std::uint64_t>(a) | (static_cast<std::uint64_t>(b) << 16) |
           (static_cast<std::uint64_t>(c) << 32) | (static_cast<std::uint64_t>(d) << 48);
  };
...
TEST_CASE("e2e: PMOVMSKB eax, xmm0 — F2-IR-027 byte-MSB extraction") {
  ...
  //   lo bytes 0..7: 0x80 (MSB=1), 0x01 (MSB=0), 0xFF (1), 0x7F (0), 0x80 (1), 0x00 (0), 0xC0 (1),
  //   0x40 (0) hi bytes 8..15: 0x80, 0x80, 0x00, 0x00, 0xFF, 0xFF, 0x01, 0x80
...
TEST_CASE("e2e: PMULHW xmm0, xmm1 — F2-IR-025 signed high-half multiply") {
  ...
  runtime::Dispatcher disp{tx, reader};
  disp.install_halt_return_stack();
  auto pack4h = [](std::int16_t a, std::int16_t b, std::int16_t c,
                   std::int16_t d) -> std::uint64_t {
...
TEST_CASE("e2e: PMULLW xmm0, xmm1 — F2-IR-013 lane-wise i16 multiply") {
  ...
  runtime::Dispatcher disp{tx, reader};
  disp.install_halt_return_stack();
  auto pack8 = [](std::uint16_t a, std::uint16_t b, std::uint16_t c,
                  std::uint16_t d) -> std::uint64_t {
    return static_cast<std::uint64_t>(a) | (static_cast<std::uint64_t>(b) << 16) |
           (static_cast<std::uint64_t>(c) << 32) | (static_cast<std::uint64_t>(d) << 48);
  };
...
TEST_CASE("e2e: PADDD xmm0, [rcx] — F2-IR-007 SSE2 memory operand") {
  ...
  disp.state().xmm[0].lo = pack4(1u, 2u);
  disp.state().xmm[0].hi = pack4(3u, 4u);
  disp.state()[ir::Gpr::Rcx] = reinterpret_cast<std::uint64_t>(&mem_operand[0]);
...
TEST_CASE("e2e: cache hit — running the same blob twice reuses the translation") {
  ...
  translator::Translator tx;
  std::vector<std::uint8_t> bytes{
      0x48, 0xB8, 0x55, 0xAA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
      return {};
...
TEST_CASE("e2e: VPERM2F128 ymm0, ymm1, ymm2, 0x21 — F2-IR-005 lane swap") {
  ...
  translator::Translator tx;
  std::vector<std::uint8_t> code{
      0xC4, 0xE3, 0x75, 0x06, 0xC2, 0x21, 0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
      return {};
...
TEST_CASE("e2e: VPERM2F128 ymm0, ymm1, ymm2, 0x88 — F2-IR-005 zero both halves") {
  ...
  translator::Translator tx;
  std::vector<std::uint8_t> code{
      0xC4, 0xE3, 0x75, 0x06, 0xC2, 0x88, 0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
      return {};
...
TEST_CASE("e2e: REP MOVSB — F2-BK-009 native loop memcpy") {
  ...
  for (std::size_t i = 0; i < 24; ++i)
    REQUIRE(dst_buf[i] == src_buf[i]);
  for (std::size_t i = 24; i < 64; ++i)
    REQUIRE(dst_buf[i] == 0xCC);
  REQUIRE(disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rcx)] == 0ULL);
...
TEST_CASE("e2e: real CALL/RET round-trip — F2-IR-054 opt-in semantics") {
  ...
  INFO("dispatch: " << r.message << "  steps=" << r.stats.steps_taken);
  REQUIRE(r.exit == runtime::DispatchExit::Halted);
  // Final EAX should be 0x22 — the post-CALL marker, executed
  // *after* the callee's RET resumed at 0x400A.
  REQUIRE((disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rax)] & 0xFFFFFFFFull) == 0x22ull);
}
...
TEST_CASE("e2e: VPGATHERDD xmm1, [rax + xmm2*4], xmm3 — F2-IR-059 masked gather") {
  ...
  alignas(16) std::array<std::uint32_t, 8> table{100u, 101u, 102u, 103u, 104u, 105u, 106u, 107u};
...
TEST_CASE("e2e: VPGATHERDQ xmm1, [rax + xmm2*8], xmm3 — F2-IR-059 qword gather") {
  ...
  alignas(16) std::array<std::uint64_t, 4> table{1000ull, 1001ull, 1002ull, 1003ull};
  disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rax)] =
      reinterpret_cast<std::uint64_t>(table.data());
...
TEST_CASE("e2e: VPGATHERQQ xmm1, [rax + xmm2*8], xmm3 — F2-IR-059 qword indices") {
  ...
  alignas(16) std::array<std::uint64_t, 4> table{2000ull, 2001ull, 2002ull, 2003ull};
  disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rax)] =
      reinterpret_cast<std::uint64_t>(table.data());
...
TEST_CASE("e2e: VGATHERDPS xmm1, [rax + xmm2*4], xmm3 — F2-IR-059 FP gather") {
  ...
  alignas(16) std::array<std::uint32_t, 8> table{0x3F80'0000u, 0x3FC0'0000u, 0x4000'0000u,
                                                 0x4040'0000u, 0x4080'0000u, 0x40A0'0000u,
                                                 0x40C0'0000u, 0x40E0'0000u};
  disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rax)] =
      reinterpret_cast<std::uint64_t>(table.data());
  ...
  auto r = disp.run(0x4000, 100);
  REQUIRE(r.exit == runtime::DispatchExit::Halted);
  REQUIRE(disp.state().xmm[1].lo == ((0xAAAA'AAAAull << 32) | 0x40C0'0000ull));  // table[6]
  REQUIRE(disp.state().xmm[1].hi == ((0xCCCC'CCCCull << 32) | 0x3FC0'0000ull));  // table[1]
  REQUIRE(disp.state().xmm[3].lo == 0u);
  REQUIRE(disp.state().xmm[3].hi == 0u);
  REQUIRE(disp.state().ymm_hi[1].lo == 0u);
}
...
TEST_CASE("e2e: VPGATHERDD ymm1, [rax + ymm2*4], ymm3 — F2-IR-059 ymm gather") {
  ...
  REQUIRE(disp.state().ymm_hi[1].lo == ((0xEEEE'EEEEull << 32) | 101ull));
  REQUIRE(disp.state().ymm_hi[1].hi == ((0x1111'1111ull << 32) | 105ull));
...
inline V4 ref_sha1rnds4(const V4& s, const V4& m, unsigned imm) {
  ...
  static constexpr std::uint32_t K[4] = {0x5A827999u, 0x6ED9EBA1u, 0x8F1BBCDCu, 0xCA62C1D6u};
...
inline V4 ref_sha256rnds2(const V4& cdgh, const V4& abef, std::uint32_t wk0, std::uint32_t wk1) {
...
inline V4 ref_sha256msg1(const V4& a, const V4& b) {
  return {a[0] + sig0(a[1]), a[1] + sig0(a[2]), a[2] + sig0(a[3]),
          a[3] + sig0(b[0])};  // b lanes 1..3 ignored
}
...
inline Result run(std::vector<std::uint8_t> body, Xmm x0, Xmm x1, Xmm x2) {
  translator::Translator tx;
  ...
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    ...
    if (off >= body.size())
      return {};
    return std::span<const std::uint8_t>(body.data() + off, body.size() - off);
  };
  runtime::Dispatcher disp{tx, reader};
  disp.install_halt_return_stack();
  disp.state().xmm[0].lo = x0.first;
  disp.state().xmm[0].hi = x0.second;
  disp.state().xmm[1].lo = x1.first;
  disp.state().xmm[1].hi = x1.second;
  disp.state().xmm[2].lo = x2.first;
  disp.state().xmm[2].hi = x2.second;
  auto r = disp.run(0x4000, 100);
...
TEST_CASE("e2e: SHA1RNDS4 all selectors vs SDM reference — F2-IR-060") {
  ...
  for (unsigned imm = 0; imm < 4; ++imm) {
    ...
    auto r = run({0x0F, 0x3A, 0xCC, 0xCA, static_cast<std::uint8_t>(imm)}, {0, 0}, pack(state),
                 pack(msg));
    REQUIRE(r.halted);
    REQUIRE(r.xmm1 == pack(ref_sha1rnds4(state, msg, imm)));
  }
  // imm8 upper bits are ignored: 0xE6 behaves as 2.
  auto r = run({0x0F, 0x3A, 0xCC, 0xCA, 0xE6}, {0, 0}, pack(state), pack(msg));
  REQUIRE(r.halted);
...
TEST_CASE("e2e: SHA256RNDS2 + ping-pong chaining vs reference — F2-IR-060") {
  ...
  const Xmm x0 = {(static_cast<std::uint64_t>(wk1) << 32) | wk0, 0xDEADBEEF'CAFEF00Dull};
...
inline void emit_mem(Bytes& o, unsigned reg, unsigned base, std::uint32_t disp) {
...
inline void movdqu_load(Bytes& o, unsigned x, unsigned base, std::uint32_t d) {
  o.insert(o.end(), {0xF3, 0x0F, 0x6F});
  emit_mem(o, x, base, d);
}
inline void movdqa_store(Bytes& o, unsigned x, unsigned base, std::uint32_t d) {
  o.insert(o.end(), {0x66, 0x0F, 0x7F});
  emit_mem(o, x, base, d);
}
...
inline void paddd_mem(Bytes& o, unsigned dst, unsigned base, std::uint32_t d) {
  o.insert(o.end(), {0x66, 0x0F, 0xFE});
  emit_mem(o, dst, base, d);
}
inline void pshufd_rr(Bytes& o, unsigned dst, unsigned src, std::uint8_t imm) {
  o.insert(o.end(), {0x66, 0x0F, 0x70, modrm_rr(dst, src), imm});
}
inline void palignr_rr(Bytes& o, unsigned dst, unsigned src, std::uint8_t imm) {
...
inline void sha1rnds4(Bytes& o, unsigned dst, unsigned src, std::uint8_t imm) {
...
inline void sha1nexte_mem(Bytes& o, unsigned dst, unsigned base, std::uint32_t d) {
  o.insert(o.end(), {0x0F, 0x38, 0xC8});
  emit_mem(o, dst, base, d);
}
...
// 16 per block.
inline std::vector<std::uint32_t> pad_to_words(const char* msg, std::size_t len) {
  ...
  for (std::size_t i = 0; i < w.size(); ++i) {
    w[i] = (std::uint32_t{bytes[4 * i]} << 24) | (std::uint32_t{bytes[4 * i + 1]} << 16) |
           (std::uint32_t{bytes[4 * i + 2]} << 8) | std::uint32_t{bytes[4 * i + 3]};
  }
  return w;
}
...
inline V4 unpack(std::uint64_t lo, std::uint64_t hi) {
  return {static_cast<std::uint32_t>(lo), static_cast<std::uint32_t>(lo >> 32),
...
inline void host_sha256(const std::uint32_t* w, unsigned blocks, V4& abef, V4& cdgh) {
  ...
  for (unsigned b = 0; b < blocks; ++b) {
    ...
    for (unsigned g = 0; g < 16; ++g) {
      const V4 k = {kSha256K[4 * g], kSha256K[4 * g + 1], kSha256K[4 * g + 2], kSha256K[4 * g + 3]};
      ...
      if (g < 12) {
        ...
        m[g % 4] =
            ref_sha256msg2(lane_add(ref_sha256msg1(m[g % 4], m[(g + 1) % 4]), t), m[(g + 3) % 4]);
      }
    }
...
inline void host_sha1(const std::uint32_t* w, unsigned blocks, V4& abcd, V4& e_vec) {
  ...
  for (unsigned b = 0; b < blocks; ++b) {
    ...
    for (unsigned g = 0; g < 20; ++g) {
      ...
      if (g < 16) {
        m[g % 4] = ref_sha1msg2(lane_xor(ref_sha1msg1(m[g % 4], m[(g + 1) % 4]), m[(g + 2) % 4]),
                                m[(g + 3) % 4]);
      }
    }
...
inline JitState run_kat(Bytes body, std::uint64_t k_ptr, std::uint64_t msg_ptr,
                        std::uint64_t scratch_ptr, sha_e2e::Xmm x1, sha_e2e::Xmm x2) {
  translator::Translator tx;
  body.push_back(0xC3);  // ret
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
...
constexpr const char* kTwoBlockMsg = "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";

constexpr Kat kKats[3] = {{"", 0}, {"abc", 3}, {kTwoBlockMsg, 56}};

constexpr std::uint32_t kSha256Expected[3][8] = {
    {0xE3B0C442u, 0x98FC1C14u, 0x9AFBF4C8u, 0x996FB924u, 0x27AE41E4u, 0x649B934Cu, 0xA495991Bu,
     0x7852B855u},
    {0xBA7816BFu, 0x8F01CFEAu, 0x414140DEu, 0x5DAE2223u, 0xB00361A3u, 0x96177A9Cu, 0xB410FF61u,
     0xF20015ADu},
    {0x248D6A61u, 0xD20638B8u, 0xE5C02693u, 0x0C3E6039u, 0xA33CE459u, 0x64FF2167u, 0xF6ECEDD4u,
     0x19DB06C1u}};
...
TEST_CASE(
    "e2e: SHA-256 full-digest KAT via canonical SHA-NI loop "
    "— F2-IR-060") {
  ...
  for (std::size_t k = 0; k < 3; ++k) {
    ...
    const std::uint32_t host_digest[8] = {abef[3], abef[2], cdgh[3], cdgh[2],
    ...
    auto r = run_kat(
        body, reinterpret_cast<std::uint64_t>(kSha256K), reinterpret_cast<std::uint64_t>(w.data()),
        reinterpret_cast<std::uint64_t>(scratch), sha_e2e::pack(iv_abef), sha_e2e::pack(iv_cdgh));
    REQUIRE(r.halted);
    const std::uint32_t jit_digest[8] = {r.xmm1[3], r.xmm1[2], r.xmm2[3], r.xmm2[2],
...
TEST_CASE(
    "e2e: SHA-1 full-digest KAT via canonical SHA-NI loop "
    "— F2-IR-060") {
  using namespace sha_kat;
  ...
  for (std::size_t k = 0; k < 3; ++k) {
    ...
    const std::uint32_t host_digest[5] = {abcd[3], abcd[2], abcd[1], abcd[0], e_vec[3]};
    for (int i = 0; i < 5; ++i) {
    ...
    auto r = run_kat(body, /*k_ptr=*/0, reinterpret_cast<std::uint64_t>(guest_w.data()),
                     reinterpret_cast<std::uint64_t>(scratch), sha_e2e::pack(iv_abcd),
                     sha_e2e::pack(iv_e));
    REQUIRE(r.halted);
    const std::uint32_t jit_digest[5] = {r.xmm1[3], r.xmm1[2], r.xmm1[1], r.xmm1[0], r.xmm2[3]};
    for (int i = 0; i < 5; ++i) {
...
TEST_CASE("e2e: CPUID leaf model + SHA advertisement — F2-IR-060 followup") {
  ...
  SECTION("leaf 1 reports signature + honest feature bits") {
    ...
    REQUIRE(s[ir::Gpr::Rdx] ==
            ((1u << 0) | (1u << 4) | (1u << 8) | (1u << 15) | (1u << 25) | (1u << 26)));
...
TEST_CASE("e2e: RDTSC is monotonic and non-zero — decoder gap sweep") {
  ...
  static const std::vector<std::uint8_t> body{
      0x0F, 0x31, 0x48, 0x89, 0xC3,  // mov rbx, rax
      0x0F, 0x31, 0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
  ...
  const std::uint64_t second = disp.state()[ir::Gpr::Rax] | (disp.state()[ir::Gpr::Rdx] << 32);
  REQUIRE(second != 0u);
...
TEST_CASE("e2e: CMPXCHG8B success and failure paths + ZF direction") {
  ...
  auto run_c8b = [](std::uint64_t mem_init, std::uint32_t eax, std::uint32_t edx) {
    translator::Translator tx;
    ...
    return Out{mem, disp.state()[ir::Gpr::Rdi], disp.state()[ir::Gpr::Rax],
...
TEST_CASE("e2e: CMPXCHG with rm aliasing RAX takes the dst write") {
  ...
  static const std::vector<std::uint8_t> body{0x48, 0x0F, 0xB1, 0xC8, 0xC3};
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
      return {};
    const std::size_t off = static_cast<std::size_t>(pc - 0x4000ull);
...
TEST_CASE("e2e: BSF/BSR real results + src==0 preserves dst") {
  ...
  auto run_scan = [](std::vector<std::uint8_t> body, std::uint64_t rax, std::uint64_t rcx) {
    translator::Translator tx;
    body.push_back(0xC3);
    auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
      if (pc < 0x4000ull)
  ...
  REQUIRE(run_scan({0x48, 0x0F, 0xBD, 0xC1}, 0xDEAD, 0x8000000000000000ull) == 63u);
...
TEST_CASE("e2e: POPCNT 32-bit form counts and sets ZF") {
  ...
  static const std::vector<std::uint8_t> body{
      ...
      0x74, 0x0B,                                   // jz +11 (to movabs rdi,2)
      0x48, 0xBF, 0x01, 0,    0, 0, 0, 0, 0, 0,     // movabs rdi, 1
      0xC3, 0x48, 0xBF, 0x02, 0, 0, 0, 0, 0, 0, 0,  // movabs rdi, 2
      0xC3,
  };
  auto reader = [&](std::uint64_t pc) -> std::span<const std::uint8_t> {
    if (pc < 0x4000ull)
...
TEST_CASE("e2e: VPMOVZXBW ymm0, xmm1 — F2-IR-005 byte→word zero-extend ymm") {
  ...
  auto pack4w = [](std::uint16_t a, std::uint16_t b, std::uint16_t c,
                   std::uint16_t d) -> std::uint64_t {
    return static_cast<std::uint64_t>(a) | (static_cast<std::uint64_t>(b) << 16) |
           (static_cast<std::uint64_t>(c) << 32) | (static_cast<std::uint64_t>(d) << 48);
  };
...
TEST_CASE("e2e: MUL r/m64 produces RDX:RAX 128-bit product (F2-BK-007)") {
  ...
  INFO("dispatch: " << r.message);
  REQUIRE(r.exit == runtime::DispatchExit::Halted);
  REQUIRE(disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rax)] == 0xFFFFFFFFFFFFFFFEULL);
...
TEST_CASE("e2e: DIV r/m64 → quotient in RAX, remainder in RDX (F2-BK-007)") {
  ...
  disp.state().gpr[static_cast<std::size_t>(ir::Gpr::Rdx)] = 0ULL;
  auto r = disp.run(0x4000, 100);
  INFO("dispatch: " << r.message);
  REQUIRE(r.exit == runtime::DispatchExit::Halted);
  // 100 / 7 = 14, 100 % 7 = 2
