...
|---|---|
| `[ ]` | TODO, unclaimed |
| `[~|Codex]` or `[~|codex]` | IN PROGRESS, claimed |
...
- Respecting territory split: codex owns decoder/IR/dispatcher, Codex owns emitter/passes/lowerer/cache/runtime/infra-CI.
...
1. Edit `docs/BACKLOG.md` — flip the chosen line:
   ...
   to
   ```
   - [~|Codex] F1-XX-NNN: <title>
   ```
2. Commit **only the backlog change**:
   ```
   chore(backlog): Codex claims F1-XX-NNN: <title>
   ```
...
Per AGENTS.md commit discipline:
- English commits.
...
2. **Separate commit**: flip `[~|Codex]` to `[x] (<sha>)` in `docs/BACKLOG.md`, where `<sha>` is the short hash of the implementation commit (or the first commit in a multi-commit series).
...
2. Commit: `chore(backlog): Codex unclaims F1-XX-NNN — <reason>`.
