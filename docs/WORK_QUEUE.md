...
Last updated: 2026-06-11 (direct-exit patch lifecycle landed).
...
Latest local validation: 997/997 (`~signal_handler*`) on the mounted
`prisma-build-env` container after `2ad2e26`.
...
| 15 | Direct branch threading | 4-6 commits | `b1e112f`, `194fcca`, `b7e4cfd`, `2ad2e26` | 🟡 partial | Stage 2 threads direct CallRel and REP clamp re-entry/fallthrough successors through the hash-checked dispatcher path. Stage 3 adds the safe AArch64 branch patch primitive in `JitSlabPool`; Stage 4 emits patchable tail slots + Translator metadata for JumpRel/CallRel; Stage 5 adds Translator patch/unpatch lifecycle + stale incoming unpatch on SMC retranslation. Automatic dispatcher patching still queued until patched-chain execution preserves halt/max-step/RAS visibility. |
