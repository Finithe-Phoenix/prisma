...
// Fast-path page-cache allocation inside mheap.allocSpan. The call boundary
// exposes the P-local pageCache and its free bitmap; the return boundary
// exposes the selected base after the cache bits have been cleared. Preserve
// this causal source because small manual stack spans do not necessarily enter
...
// Return address of the slow allocator call from the same small-scan path. The
// object and mspan are in RAX/RBX; the caller-saved request, type and mcache
// remain in the stack slots populated immediately before the call.
#[cfg(any(test, target_arch = "arm64ec"))]
...
impl StackcacheLinkWatch {
        ...
        F: FnMut(u64) -> Option<u64>,
    {
        let address = self.address.load(Ordering::SeqCst);
        let rsp = frame.gpr[gpr::RSP];
        let (result, span, requested_size, allocation_type, mcache) = match guest_rip {
            ...
            GO_MALLOCGC_SMALL_SCAN_SLOW_RESULT_RIP => (
                frame.gpr[gpr::RAX],
                frame.gpr[gpr::RBX],
                read_u64(rsp.checked_add(0x60)?).unwrap_or(INVALID_MORESTACK_MEMORY),
                read_u64(rsp.checked_add(0x68)?).unwrap_or(INVALID_MORESTACK_MEMORY),
                read_u64(rsp.checked_add(0x30)?).unwrap_or(INVALID_MORESTACK_MEMORY),
            ),
            _ => return None,
        };
...
#[cfg(any(test, target_arch = "arm64ec"))]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
struct PageCacheOrigin {
    mspan: u64,
    start: u64,
    allocation: PageCacheAllocation,
}
...
struct PallocOriginState {
    ...
    page_cache_sequence: u64,
    page_cache_cursor: usize,
    pending_refill_base: u64,
    pending_refill_cache: u64,
    pending_refill_scav: u64,
    page_cache_allocations: [PageCacheAllocation; PALLOC_ORIGIN_SLOT_COUNT],
    page_cache_origin_cursor: usize,
    page_cache_origins: [PageCacheOrigin; PALLOC_ORIGIN_SLOT_COUNT],
}
...
impl PallocOriginState {
    const fn new() -> Self {
        Self {
            ...
            page_cache_sequence: 0,
            page_cache_cursor: 0,
            pending_refill_base: 0,
            pending_refill_cache: 0,
            pending_refill_scav: 0,
            page_cache_allocations: [PageCacheAllocation {
                sequence: 0,
                g: 0,
                page_cache: 0,
                cache_base: 0,
            ...
            page_cache_origin_cursor: 0,
            page_cache_origins: [PageCacheOrigin {
                mspan: 0,
                start: 0,
                allocation: PageCacheAllocation {
                    sequence: 0,
...
impl PallocOriginTracker {
    ...
    fn record_page_cache_allocation_with_reader<F>(
        &self,
        guest_rip: u64,
        frame: &CpuStateFrame,
        mut read_u64: F,
        ...
        F: FnMut(u64) -> Option<u64>,
    {
        ...
        if guest_rip == GO_PAGE_CACHE_ALLOC_CALL_RIP {
            let page_cache = frame.gpr[gpr::RAX];
            let npages = frame.gpr[gpr::RBX];
            if g == 0 || page_cache == 0 || npages == 0 {
                return;
            }
            let cache_base = read_u64(page_cache).unwrap_or(INVALID_MORESTACK_MEMORY);
            let cache_before = page_cache
                .checked_add(8)
            ...
            let refill_cache = if state.pending_refill_base == cache_base
                && state.pending_refill_cache == cache_before
                && state.pending_refill_scav == scav_before
            {
                state.pending_refill_cache
            } else {
                INVALID_MORESTACK_MEMORY
            };
            state.pending_refill_base = 0;
            state.pending_refill_cache = 0;
            state.pending_refill_scav = 0;
        ...
        let allocated_base = frame.gpr[gpr::RAX];
        if allocated_base == 0 {
            return;
        }
        let Ok(mut state) = self.state.lock() else {
        ...
        let Some(index) = state
            ...
            .enumerate()
            .filter(|(_, allocation)| {
                allocation.sequence != 0 && allocation.g == g && allocation.allocated_base == 0
            })
        ...
        let page_cache = state.page_cache_allocations[index].page_cache;
        let cache_after = page_cache
            .checked_add(8)
            .and_then(&mut read_u64)
            .unwrap_or(INVALID_MORESTACK_MEMORY);
    ...
    fn record_page_cache_refill(&self, guest_rip: u64, frame: &CpuStateFrame) {
        ...
        if cache_base == 0 || refill_cache == 0 {
            return;
        }
        let Ok(mut state) = self.state.lock() else {
            return;
        };
        state.pending_refill_base = cache_base;
        state.pending_refill_cache = refill_cache;
        state.pending_refill_scav = refill_scav;
    }
    ...
    fn associate_page_cache_origin(&self, mspan: u64, start: u64) {
        ...
        let Some(allocation) = state
            .page_cache_allocations
            .iter()
            .filter(|allocation| allocation.sequence != 0 && allocation.allocated_base == start)
            .max_by_key(|allocation| allocation.sequence)
            .copied()
        else {
            return;
        };
        let index = state.page_cache_origin_cursor % PALLOC_ORIGIN_SLOT_COUNT;
        state.page_cache_origins[index] = PageCacheOrigin {
            mspan,
            start,
            allocation,
        };
    ...
    fn page_cache_origin_event(&self, mspan: u64, watched_address: u64) -> Option<MorestackEvent> {
        ...
        Some(MorestackEvent {
            rip: GO_PAGE_CACHE_ALLOC_RETURN_RIP,
            r14: watched_address,
        ...
        F: FnMut(u64) -> Option<u64>,
    {
        ...
        let mut event = self
            ...
            .or_else(|| self.origin_event(mspan, watched_address))
    ...
    fn associate_manual_span(&self, g: u64, mspan: u64, start: u64) {
        ...
        let any_g_snapshot = state
            .snapshots
            .iter()
            .filter(|snapshot| snapshot.sequence > call.palloc_sequence)
            .max_by_key(|snapshot| snapshot.sequence)
            .copied();
        // Status zero is a usable origin. Persist failed association attempts
        // too: 3=same-g range miss, 7=only a different-g candidate survived,
        // 8=the global sequence advanced but no after-call snapshot survived.
        let (status, snapshot) = covering_snapshot.map_or_else(
            || {
                same_g_snapshot.map_or_else(
                    || {
                        any_g_snapshot.map_or_else(
                            || (8, PallocRangeSnapshot::default()),
                            |snapshot| (7, snapshot),
                        )
                    },
                    |snapshot| (3, snapshot),
    ...
    fn origin_miss_event(
        ...
        let observed_sequence =
            retained_origin.map_or(state.palloc_sequence, |origin| origin.observed_sequence);
        let call_sequence = retained_origin.map_or_else(
            || pending.map_or(INVALID_MORESTACK_MEMORY, |call| call.palloc_sequence),
            |origin| origin.call_sequence,
        );
        Some(MorestackEvent {
            rip: GO_PALLOC_DATA_ALLOC_RANGE_RIP,
            r14: watched_address,
...
mod tests {
    ...
    #[test]
    fn palloc_origin_tracker_associates_page_cache_fast_path_at_creation() {
        let tracker = PallocOriginTracker::new();
        let mut frame = CpuStateFrame::default();
        frame.gpr[gpr::R14] = 0x7000;
        tracker.record_stackpool_call(GO_STACKPOOL_CREATE_CALL_RIP, &frame);
        ...
        frame.gpr[gpr::RAX] = 0x1c60_abe1_0000;
        tracker.record_page_cache_allocation_with_reader(
            GO_PAGE_CACHE_ALLOC_RETURN_RIP,
            &frame,
            |address| (address == 0xa008).then_some(0x1000),
        );
        ...
        let creation_event = tracker
            .associate_stackpool_creation_with_reader(
                GO_STACKPOOL_CREATE_RETURN_RIP,
                &frame,
                |address| (address == 0x8018).then_some(0x1c60_abe1_0000),
            )
            .unwrap();
        assert_eq!(creation_event.rip, GO_STACKPOOL_CREATE_RETURN_RIP);
        assert_eq!(creation_event.r14, 0x1c60_abe1_2000);
        assert_eq!(creation_event.rax, 0xa000);
        assert_eq!(creation_event.rbx, 0x1c60_abe0_0000);
        assert_eq!(creation_event.rcx, 4);
        assert_eq!(creation_event.rsp, 1);
        assert_eq!(creation_event.r8, 0x8000);
        ...
        assert_eq!(creation_event.rcx_stack_hi, 0);

        let origin = tracker
            .page_cache_origin_event(0x8000, 0x1c60_abe1_2000)
            .unwrap();
    ...
    fn palloc_origin_tracker_retains_refill_bitmap_on_next_allocation() {
        ...
        tracker.record_page_cache_refill(GO_PAGE_CACHE_REFILL_RETURN_RIP, &frame);

        frame.gpr[gpr::R14] = 0x7100;
        frame.gpr[gpr::RAX] = page_cache;
        frame.gpr[gpr::RBX] = 1;
        let before = BTreeMap::from([
            (page_cache, cache_base),
            (page_cache + 8, 0x2f),
            (page_cache + 16, 0x20),
        ]);
        tracker.record_page_cache_allocation_with_reader(
    ...
    #[test]
    fn palloc_origin_tracker_retains_different_g_association_failure() {
        let tracker = PallocOriginTracker::new();
        let mut frame = CpuStateFrame::default();
        frame.gpr[gpr::R14] = 0x7000;
        tracker.record_stackpool_call(GO_STACKCACHE_REFILL_POOL_CALL_RIP, &frame);
        ...
        let event = tracker
            ...
            .unwrap();
        assert_eq!(event.rax, 0);
        assert_eq!(event.rbx, 7);
        assert_eq!(event.rcx, 1);
        assert_eq!(event.rsp, 0);
        assert_eq!(event.rax_stack_lo, 1);
        assert_eq!(event.rax_stack_hi, 1);
        assert_eq!(event.rcx_stack_lo, 1);
        assert_eq!(event.rcx_stack_hi, 256);
    }

    #[test]
    fn stackcache_link_watch_reports_fast_allocator_alias_before_mutation() {
        let watch = StackcacheLinkWatch::new();
        watch.arm_from_refill(MorestackEvent {
            rip: GO_STACKCACHE_REFILL_LOOP_STATE_RIP,
            rax: 0x4000,
    ...
    #[test]
    fn stackcache_link_watch_reports_slow_allocator_alias_before_mutation() {
        let watch = StackcacheLinkWatch::new();
        watch.arm_from_refill(MorestackEvent {
            rip: GO_STACKCACHE_REFILL_LOOP_STATE_RIP,
            rax: 0x4000,
        ...
        let mut frame = CpuStateFrame::default();
        frame.gpr[gpr::R14] = 0x1000;
        frame.gpr[gpr::RAX] = 0x4100;
        frame.gpr[gpr::RBX] = 0x6000;
        frame.gpr[gpr::RSP] = 0x3000;
        let memory = BTreeMap::from([
            (0x3030, 0x5000),
            (0x3060, 0x10),
        ...
        let event = watch
            ...
            .unwrap();
        assert_eq!(event.rip, GO_MALLOCGC_SMALL_SCAN_SLOW_RESULT_RIP);
        assert_eq!(event.r14, 0x1000);
        assert_eq!(event.rax, 0x4100);
        assert_eq!(event.rbx, 0x6000);
        assert_eq!(event.rcx, 0x10);
